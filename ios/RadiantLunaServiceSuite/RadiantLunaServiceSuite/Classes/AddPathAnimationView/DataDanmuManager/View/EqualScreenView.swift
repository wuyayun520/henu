...
fileprivate let const_clickPath:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "move" :*/
fileprivate let data_fieldUrl:String = "minfove"
...
class EqualScreenView: UIView {
    ...
    var danumuMsgArray: [MaxMeasurable] = []
    //: var danumuItemViewArray: [TalkingFloatScreenItemView] = []
    var danumuItemViewArray: [CommonReactiveCompatible] = []

    //: let TapAction: PublishSubject<TalkingFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<MaxMeasurable?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_clickPath.reversed(), encoding: .utf8)!)
    }
    ...
    func messageVideo(_ danumuMsgModel: MaxMeasurable?) {
    ...
    @objc func strikeDown(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(strikeDown(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension EqualScreenView: UIGestureRecognizerDelegate {
    ...
    func overUser(for danumuMsgModel: MaxMeasurable?, isFirst: Bool) {
        //: let itemView = TalkingFloatScreenItemView(model: danumuMsgModel)
        let itemView = CommonReactiveCompatible(model: danumuMsgModel)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (data_fieldUrl.replacingOccurrences(of: "info", with: "o")))
    ...
    func afterSubChannel(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                overUser(for: model, isFirst: isFirst)
...
extension EqualScreenView {
    // 添加视图
    //: private func setupSubviews() {
    private func dominion() {
