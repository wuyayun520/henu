...
fileprivate let userLabTitle:[UInt8] = [0x23,0x24,0x23,0x3e,0x62,0x29,0x25,0x2e,0x2f,0x38,0x70,0x63,0x6a,0x22,0x2b,0x39,0x6a,0x24,0x25,0x3e,0x6a,0x28,0x2f,0x2f,0x24,0x6a,0x23,0x27,0x3a,0x26,0x2f,0x27,0x2f,0x24,0x3e,0x2f,0x2e]

/*: "#AAAAAA" :*/
fileprivate let kTopDataAddMessage:String = "#AAAAAAtarget video content"

/*: "btn_chatsettings_unchoice" :*/
fileprivate let show_actualPath:String = "half first arraybtn_ch"
fileprivate let const_backKey:String = "ttoolg"
fileprivate let kDetailStr:String = "hlabelce"

/*: "btn_chatsettings_choiced" :*/
fileprivate let kTitleId:[Character] = ["b","t","n","_","c","h","a","t","s","e","t","t","i","n","g","s","_","c","h"]
fileprivate let showLabFormat:String = "ocolored"

/*: "Save" :*/
fileprivate let notiMakeStr:[Character] = ["S","a","v","e"]

/*: "Cancel" :*/
fileprivate let constUntilId:[Character] = ["C","a","n","c","e","l"]
...
class AppView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userLabTitle.map{$0^74}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIView = {
        ...
        let height = actualWidth(w: 475) + noti_saveFormat
        //: let v = UIView(frame: CGRect(x: 0, y: ScreenHeight-height, width: ScreenWidth, height: height))
        let v = UIView(frame: CGRect(x: 0, y: constRecordServerKey - height, width: main_colorData, height: height))
        //: v.backgroundColor = .white
        v.backgroundColor = .white
        //: v.Corner(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        v.meanSunRadii(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var textBgView: UIView = {
        ...
        v.backgroundColor = UIColor.pushAside()
    ...
    private lazy var limitLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(kTopDataAddMessage.prefix(7))))
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var topBtn: BlockShouldShirtButton = {
        ...
        btn.setImage(UIImage.bundle(name: (String(show_actualPath.suffix(6)) + "atset" + const_backKey.replacingOccurrences(of: "tool", with: "in") + "s_unc" + kDetailStr.replacingOccurrences(of: "label", with: "oi"))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btn_chatsettings_choiced"), for: .selected)
        btn.setImage(UIImage.bundle(name: (String(kTitleId) + showLabFormat.replacingOccurrences(of: "color", with: "ic"))), for: .selected)
        ...
        btn.addTarget(self, action: #selector(sortClick), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var saveBtn: UIButton = {
        ...
        btn.setTitle((String(notiMakeStr)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(sumClearAdd), for: .touchUpInside)
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        btn.setTitle((String(constUntilId)).localized, for: .normal)
...
extension AppView: YYTextViewDelegate {
...
extension AppView {
    ...
    @objc private func sumClearAdd() {
        //: saveBtn.isEnabled = false
        saveBtn.isEnabled = false
        //: let isTop = topBtn.isSelected ? 1 : 2
        let isTop = topBtn.isSelected ? 1 : 2
        //: StreamRequestTool.req_remark(content: textView.text, isTop: isTop, toUid: targetId) {
        StreamRequestTool.frontCompletion(content: textView.text, isTop: isTop, toUid: targetId) {
            //: succeed, _, _ in
            succeed, _, _ in
    ...
    func demonstrateUid(_ content: String, isTop: Int, toUid: String) {
        //: targetId = toUid
        targetId = toUid
