...
fileprivate let const_viewData:[UInt8] = [0x98,0x9f,0x98,0x85,0xd9,0x92,0x9e,0x95,0x94,0x83,0xcb,0xd8,0xd1,0x99,0x90,0x82,0xd1,0x9f,0x9e,0x85,0xd1,0x93,0x94,0x94,0x9f,0xd1,0x98,0x9c,0x81,0x9d,0x94,0x9c,0x94,0x9f,0x85,0x94,0x95]

/*: "View read receipts?" :*/
fileprivate let notiEqualPath:[Character] = ["V","i","e","w"," ","r","e","a","d"," ","r","e","c","e","i"]
fileprivate let notiInputFormat:[Character] = ["p","t","s","?"]

/*: "text" :*/
fileprivate let constMakeFormat:[Character] = ["t","e","x","t"]

/*: "color" :*/
fileprivate let constLabelUrl:[UInt8] = [0x43,0x4f,0x4c,0x4f,0x52]

				private func equalUser(min num: UInt8) -> UInt8 {
					return num ^ 32
				}

/*: "<xml>%@</xml>" :*/
fileprivate let constHiddenFormat:[Character] = ["<","x","m","l",">","%","@","<","/","x","m","l",">"]

/*: "href" :*/
fileprivate let main_colorShowTitle:String = "HREF"

/*: "type" :*/
fileprivate let mainMomentMsg:[UInt8] = [0x22,0x2f,0x26,0x33]

/*: "padding" :*/
fileprivate let user_errorText:[Character] = ["p","a","d","d","i","n"]
fileprivate let const_requestMsg:String = "user"

/*: "size" :*/
fileprivate let data_quitText:[Character] = ["s","i","z","e"]

/*: "maxFont" :*/
fileprivate let appEffectMsg:String = "maxFontprocessor return"

/*: "link" :*/
fileprivate let k_onId:String = "LINK"

/*: "PingFangSC-Regular" :*/
fileprivate let const_colorData:String = "PingFsuccessfully let"
fileprivate let constPinStr:String = "self self click-Regula"
fileprivate let const_alreadyKey:[Character] = ["r"]

/*: "content" :*/
fileprivate let constCardMsg:[UInt8] = [0x74,0x6e,0x65,0x74,0x6e,0x6f,0x63]
...
@objcMembers public class SuccessAppThen: ResponseGiftCellData {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_viewData.map{$0^241}, encoding: .utf8)!)
    }

    //: public override func heightOfWidth(_ width: CGFloat) ->CGFloat {
    override public func dimension(_: CGFloat) -> CGFloat {
        //: return contentSize().height+20
        return equalShared().height + 20
    }
    ...
    override public func equalShared() -> CGSize {
        ...
        if self.msgModel.tips.content == (String(notiEqualPath) + String(notiInputFormat)).localized {
            //: return CGSize(width: ScreenWidth, height: 30)
            return CGSize(width: main_colorData, height: 30)
        }
        ...
        //: let rect = self.attributedString.boundingRect(with: CGSize.init(width: MaleMacroDefine.getScreenWidth()-50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil )
        let rect = self.attributedString.boundingRect(with: CGSize(width: MaleMacroDefine.getFromShow() - 50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil)
    ...
    public lazy var attributedString: NSMutableAttributedString = {
        ...
        var attr = showThan(info: self.msgModel.tips)
        //: return attr
        return attr
        //: }()
    }()
...
extension SuccessAppThen {
    //: func formatMessageToStringM(info: AbTalkingChatMsgTipsModel) -> NSMutableAttributedString {
    func showThan(info: EqualShareModel) -> NSMutableAttributedString {
...
//        let dic = NSDictionary(jsonString: extra ?? "")
        ...
        return query(tips: content)
    }

    //: func designTextFream(tips: String) -> NSMutableAttributedString {
    func query(tips: String) -> NSMutableAttributedString {
        ...
        _ = buildInfo(msg: tips, info: attrbutedArray)
        //: self.yyLayout = designYYTextLayout(array: attrbutedArray, displayMaxWidth: MaleMacroDefine.getScreenWidth()-50, para: para)
        self.yyLayout = failure(array: attrbutedArray, displayMaxWidth: MaleMacroDefine.getFromShow() - 50, para: para)
    ...
    func buildInfo(msg: String, info: NSMutableArray) -> String {
        ...
        let pure_msg = tip(source: msg, attrbAM: contentAttrArray)
        //: for  textInfo in contentAttrArray {
        for textInfo in contentAttrArray {
            //: let textDic = textInfo as![String: Any]
            let textDic = textInfo as! [String: Any]
            //: let text: String = textDic["text"] as! String
            let text: String = textDic[(String(constMakeFormat))] as! String

            //: var textColor: UIColor?
            var textColor: UIColor?

            //: var obj = textDic["color"]
            var obj = textDic[String(bytes: constLabelUrl.map{equalUser(min: $0)}, encoding: .utf8)!]
            //: if obj is UIColor {
            if obj is UIColor {
                //: obj = obj as? UIColor
                obj = obj as? UIColor
            ...
            contentDict.removeObjects(forKeys: [String(bytes: constLabelUrl.map{equalUser(min: $0)}, encoding: .utf8)!, (String(constMakeFormat))])

            //: let attrDict = ctDictionaryForText(content: text, color: textColor)
            let attrDict = colorVideo(content: text, color: textColor)
    ...
    func tip(source: String, attrbAM: NSMutableArray) -> String {
        ...
        do {
            ...
            let document = try ONOXMLDocument(string: String(format: (String(constHiddenFormat)), newSource), encoding: CFStringConvertEncodingToNSStringEncoding(CFStringBuiltInEncodings.UTF8.rawValue))
            //: let element = document.rootElement
            let element = document.rootElement
            //: parseXMLElement(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: ["color": UIColor.white])
            talkAttrib(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: [String(bytes: constLabelUrl.map{equalUser(min: $0)}, encoding: .utf8)!: UIColor.white])
            ...
            attrbAM.add([(String(constMakeFormat)): newSource, String(bytes: constLabelUrl.map{equalUser(min: $0)}, encoding: .utf8)!: UIColor.white])
            //: return newSource
            return newSource
        }
    }
    ...
    func talkAttrib(element: ONOXMLElement, arrM: NSMutableArray, attrib: NSDictionary) {
        ...
        if children.count > 0 {
            ...
            if element.tag == "u" { // 下滑线
                ...
                let url: String = element.attributes[(main_colorShowTitle.lowercased())] as! String
                //: if url.count > 0 {
                if url.count > 0 {
                    //: parentInfo["href"] = url
                    parentInfo[(main_colorShowTitle.lowercased())] = url
                }
            }
            //: for  subElement in children {
            for subElement in children {
                //: parseXMLElement(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
                talkAttrib(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
            }
            ...
            if text != nil {
                ...
                //: info["text"] = text
                info[(String(constMakeFormat))] = text
                //: arrM.add(info)
                arrM.add(info)
            }
        }
    ...
    func failure(array: NSArray, displayMaxWidth: CGFloat, para: NSParagraphStyle?) -> YYTextLayout {
        //: let attrString = designAttribStr(array: array, paragraphStyle: para)
        let attrString = logSuccess(array: array, paragraphStyle: para)
    ...
    func logSuccess(array: NSArray, paragraphStyle: NSParagraphStyle?) -> NSAttributedString {
        ...
        array.enumerateObjects { obj, _, _ in
            ...
            let type = newObj[String(bytes: mainMomentMsg.map{$0^86}, encoding: .utf8)!] as? String
            //: var tmp = NSAttributedString.init()
            var tmp = NSAttributedString()
            //: if type  == "txt" {
            if type == "txt" {
                //: tmp = designAttribStrByTextDic(dict: newObj as NSDictionary)
                tmp = dataAt(dict: newObj as NSDictionary)
                //: } else if type == "padding" {
            } else if type == (String(user_errorText) + const_requestMsg.replacingOccurrences(of: "user", with: "g")) {
                //: let imageView = UIImageView.init()
                let imageView = UIImageView()
                //: imageView.size = (newObj["size"] as! NSValue).cgSizeValue
                imageView.size = (newObj[(String(data_quitText))] as! NSValue).cgSizeValue
                ...
                tmp = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: newObj[(String(appEffectMsg.prefix(7)))] as! UIFont, alignment: .center)
            }
            ...
            if underline {
                ...
                if len > 0 {
                    ...
                    _ = YYTextDecoration(style: .single, width: 1.5, color: newObj[String(bytes: constLabelUrl.map{equalUser(min: $0)}, encoding: .utf8)!] as? UIColor)
    ...
    func wayUserAppearRe(dict: NSDictionary) -> NSAttributedString {
        //: let imageView = UIImageView.init()
        let imageView = UIImageView()
        //: imageView.size = (dict["size"]as! NSValue).cgSizeValue
        imageView.size = (dict[(String(data_quitText))] as! NSValue).cgSizeValue
        ...
        let attachText = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: dict[(String(appEffectMsg.prefix(7)))] as! UIFont, alignment: .center)
        //: return attachText
        return attachText
    }

    //: func designAttribStrByTextDic(dict: NSDictionary) ->NSMutableAttributedString {
    func dataAt(dict: NSDictionary) -> NSMutableAttributedString {
        //: let type = dict["type"] as! String
        let type = dict[String(bytes: mainMomentMsg.map{$0^86}, encoding: .utf8)!] as! String
        //: if type != "link" && type != "txt" {
        if type != (k_onId.lowercased()) && type != "txt" {
            //: return NSMutableAttributedString.init()
            return NSMutableAttributedString()
        }
        //: let attributes = attributesWithConfig()
        let attributes = triviaImage()
        // set color
        //: let color = dict["color"] as? UIColor
        let color = dict[String(bytes: constLabelUrl.map{equalUser(min: $0)}, encoding: .utf8)!] as? UIColor
        //: if color != nil {
        if color != nil {
        ...
        let fontSize = (dict[(String(data_quitText))] as? NSNumber)?.floatValue
        //: if (fontSize ?? 0 > 0) {
        if fontSize ?? 0 > 0 {
            //: let fontRef = CTFontCreateWithName("PingFangSC-Regular" as CFString, CGFloat(fontSize!), nil)
            let fontRef = CTFontCreateWithName((String(const_colorData.prefix(5)) + "angSC" + String(constPinStr.suffix(7)) + String(const_alreadyKey)) as CFString, CGFloat(fontSize!), nil)
            //: attributes[kCTFontAttributeName] = fontRef
            attributes[kCTFontAttributeName] = fontRef
        }
        //: var content = (dict["content"] != nil) ? dict["content"] : ""
        var content = (dict[String(bytes: constCardMsg.reversed(), encoding: .utf8)!] != nil) ? dict[String(bytes: constCardMsg.reversed(), encoding: .utf8)!] : ""
    ...
    func triviaImage() -> NSMutableDictionary {
    ...
    func colorVideo(content: String, color: UIColor?) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
        ...
        if color == nil {
            ...
            return constraintNow(content: content)
        }
        //: return ["type": "txt",
        return [String(bytes: mainMomentMsg.map{$0^86}, encoding: .utf8)!: "txt",
                //: "content": content,
                String(bytes: constCardMsg.reversed(), encoding: .utf8)!: content,
                //: "color": color ?? .black]
                String(bytes: constLabelUrl.map{equalUser(min: $0)}, encoding: .utf8)!: color ?? .black]
    }

    //: func ctDictionaryForText(content: String) ->NSDictionary {
    func constraintNow(content: String) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
            //: return NSDictionary.init()
            return NSDictionary()
