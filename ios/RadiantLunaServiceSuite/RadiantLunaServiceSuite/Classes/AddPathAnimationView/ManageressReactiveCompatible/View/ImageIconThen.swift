...
fileprivate let noti_clickStr:[UInt8] = [0x7,0x0,0x7,0x1a,0x46,0xd,0x1,0xa,0xb,0x1c,0x54,0x47,0x4e,0x6,0xf,0x1d,0x4e,0x0,0x1,0x1a,0x4e,0xc,0xb,0xb,0x0,0x4e,0x7,0x3,0x1e,0x2,0xb,0x3,0xb,0x0,0x1a,0xb,0xa]

				private func makeIndex(shared num: UInt8) -> UInt8 {
					return num ^ 110
				}
...
class ImageIconThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_clickStr.map{makeIndex(shared: $0)}, encoding: .utf8)!)
    }
...
extension ImageIconThen {
    ...
    override func hitTest(_ point: CGPoint, with _: UIEvent?) -> UIView? {
        ...
        for subview in self.subviews {
            ...
            let itemview: GraduatedTableThen = subview as! GraduatedTableThen
            ...
            if #available(iOS 11.0, *) {
                ...
                if itemview.avatarImage.frame.contains(point) {
                    //: CombineAccountEnclaveDecisionMakerThen.share.func__pushToUserDetailVC(uid: "\(itemview.giftMsgModel.fromUid ?? 0)")
                    CombineAccountEnclaveDecisionMakerThen.share.windowBy(uid: "\(itemview.giftMsgModel.fromUid ?? 0)")
                    //: break
                    break
                }
                //: } else {
    ...
    func menuAcross(model: ViewModelType) {
        //  1.相同礼物时则连击
        //: for  subview in self.subviews {
        for subview in self.subviews {
            //: let itemview: TalkingGiftTrackItemView = subview as! TalkingGiftTrackItemView
            ...
            if highlightModel(giftMsgModel: itemview.giftMsgModel, withGiftMsgModel: model) {
                //: itemview.comboHitWith(giftMsgModel: model)
                itemview.sharedEnd(giftMsgModel: model)
                //: itemview.setGiftMsgModel(model: model)
                itemview.hideInsert(model: model)
                //: return
                return
            }
        }
        ...
        for index in 0 ... 2 {
            ...
            let subView: GraduatedTableThen? = self.viewWithTag(tag) as! GraduatedTableThen?
            //: if subView == nil {
            if subView == nil {
                //: self.addNewGiftDanumWith(giftMsgModel: model, atIndex: index)
                self.excellence(giftMsgModel: model, atIndex: index)
                //: return
                return
            }
        }
        ...
        for index in 0 ... 2 {
            ...
            if index == 1 || index == 2 {
                ...
                let subView1: GraduatedTableThen = self.viewWithTag(tag) as! GraduatedTableThen
    ...
    func highlightModel(giftMsgModel: ViewModelType, withGiftMsgModel: ViewModelType) -> (Bool) {
...
//        subView.showUserInfoViewBlock   = self.showUserInfoViewBlock
        ...
        subView.hideInsert(model: giftMsgModel)
        //: self.addSubview(subView)
        self.addSubview(subView)

        //: subView.showGiftAnimate(model: giftMsgModel, animate: true)
        subView.consultingService(model: giftMsgModel, animate: true)
    }
}
