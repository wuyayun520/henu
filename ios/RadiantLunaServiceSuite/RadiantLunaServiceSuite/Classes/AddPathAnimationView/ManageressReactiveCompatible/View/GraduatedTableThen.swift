...
fileprivate let dataViewModeOutputUrl:[UInt8] = [0x30,0x35,0x30,0x3b,0xef,0x2a,0x36,0x2b,0x2c,0x39,0x1,0xf0,0xe7,0x2f,0x28,0x3a,0xe7,0x35,0x36,0x3b,0xe7,0x29,0x2c,0x2c,0x35,0xe7,0x30,0x34,0x37,0x33,0x2c,0x34,0x2c,0x35,0x3b,0x2c,0x2b]

				fileprivate func ofAdd(text num: UInt8) -> UInt8 {
...
/*: "#FFD13A" :*/
fileprivate let appModelUrl:[Character] = ["#","F","F","D","1","3"]
fileprivate let user_imageId:String = "a"

/*: "gift_gb_umnad_1" :*/
fileprivate let data_addEqualFormat:String = "gift_gmodel layer self day"
fileprivate let showCloudValue:String = "b_uvalue video temp content of"
fileprivate let showModelData:[Character] = ["m","n","a","d","_","1"]

/*: "icon_circle_pic_default" :*/
fileprivate let data_labelText:String = "iton"
fileprivate let noti_burnUserId:String = "self table count make selecte_pic_de"
fileprivate let constCenterData:[Character] = ["f","a","u","l","t"]

/*: "Open lucky blind-box" :*/
fileprivate let noti_editId:String = "moment varOpen "
fileprivate let mainTextUrl:String = "capture layer left at runy blin"

/*: "Sent " :*/
fileprivate let main_viewDayLetMessage:[Character] = ["S","e","n","t"," "]

/*: "PingFangSC-Semibold" :*/
fileprivate let app_edgeKey:String = "Pinmanager add"
fileprivate let kWillMsg:[Character] = ["g","F","a","n","g","S","C","-","S","e","m","i","b","o","l","d"]

/*: "Bombard" :*/
fileprivate let show_dayFormat:[Character] = ["B","o","m","b","a"]
fileprivate let noti_dateFormat:String = "RD"
...
class GraduatedTableThen: UIView {
    //: var giftMsgModel = TalkingGiftMsgAnimatModel()
    var giftMsgModel = ViewModelType()
    //: var giftSendItem = [TalkingGiftNumArrModel()]
    var giftSendItem = [SessionMeasurable()]
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataViewModeOutputUrl.map{ofAdd(text: $0)}, encoding: .utf8)!)
    }
...
//        lb.numberOfLines = 1
        ...
        lb.textColor = UIColor(hex: (String(appModelUrl) + user_imageId.uppercased()))
    ...
    private lazy var pluralTip: FrameItemLabel = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = FrameItemLabel()
        //: lab.isHidden = true
        lab.isHidden = true
    ...
    private lazy var countLb: FrameItemLabel = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = FrameItemLabel()
...
extension GraduatedTableThen {
    //: func setGiftMsgModel(model: TalkingGiftMsgAnimatModel) {
    func hideInsert(model: ViewModelType) {
        ...
        self.bgImage.image = UIImage.bundle(name: (String(data_addEqualFormat.prefix(6)) + String(showCloudValue.prefix(3)) + String(showModelData)))

        //: let placeholder = UIImage.BundleImageNamed(name: "icon_circle_pic_default")
        let placeholder = UIImage.bundle(name: (data_labelText.replacingOccurrences(of: "to", with: "co") + "_circl" + String(noti_burnUserId.suffix(8)) + String(constCenterData)))

        //: if (model.showType == ChatGiftAnimatType.myStery.rawValue ) {
        if model.showType == CompartmentAdditiveArithmetic.myStery.rawValue {
            //: self.nickLb.text = "Open lucky blind-box".localized
            self.nickLb.text = (String(noti_editId.suffix(5)) + "luck" + String(mainTextUrl.suffix(6)) + "d-box").localized
            //: }else {
        } else {
        ...
        self.avatarImage.tillFirstBar(urlStr: giftMsgModel.fromHeadPic, placeImg: placeholder)
        //: self.giftImage.setUrlImage(urlStr: giftMsgModel.giftPic, placeImg: placeholder)
        self.giftImage.tillFirstBar(urlStr: giftMsgModel.giftPic, placeImg: placeholder)

        //: self.giftName.text = "Sent ".localized + giftMsgModel.pname
        self.giftName.text = (String(main_viewDayLetMessage)).localized + giftMsgModel.pname
        ...
        if sendNumber > 1 {
            ...
            var font1 = UIFont(name: (String(app_edgeKey.prefix(3)) + String(kWillMsg)), size: 22)
            //: font1 = italicFontWith(angle: 15, fontSize: 18, fontName: font1!.fontName)
            font1 = bitmap(angle: 15, fontSize: 18, fontName: font1!.fontName)
            //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 26)
            var font2 = UIFont(name: (String(app_edgeKey.prefix(3)) + String(kWillMsg)), size: 26)
            //: font2 = italicFontWith(angle: 15, fontSize: 22, fontName: font2!.fontName)
            font2 = bitmap(angle: 15, fontSize: 22, fontName: font2!.fontName)
            ...
            numText = numText.mutilpartToFontsTip(texts: strs, colors: colors, fonts: fonts)
        ...
        empty(giftCount: giftMsgModel.comboNum)
    }

    //: func refreshGiftCount(giftCount: Int) {
    func empty(giftCount: Int) {
        //: var font1 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font1 = UIFont(name: (String(app_edgeKey.prefix(3)) + String(kWillMsg)), size: 45)
        //: font1 = italicFontWith(angle: 15, fontSize: 26, fontName: font1!.fontName)
        font1 = bitmap(angle: 15, fontSize: 26, fontName: font1!.fontName)
        //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font2 = UIFont(name: (String(app_edgeKey.prefix(3)) + String(kWillMsg)), size: 45)
        //: font2 = italicFontWith(angle: 15, fontSize: 38, fontName: font2!.fontName)
        font2 = bitmap(angle: 15, fontSize: 38, fontName: font2!.fontName)
        ...
        let colors = [UIColor.roundVoice(150, 77, 20, 1), UIColor.white, UIColor.roundVoice(150, 77, 20, 1)]
        ...
        attText = attText.mutilpartToFontsTip(texts: strs, colors: colors, fonts: fonts)
        ...
        countLb.makeText(attributedText: attText)
        ...
        countLb.toBirthday(frame: CGRect(x: x, y: countLb.origin.y, width: width, height: countLb.size.height))
    }

    /// 连击
    //: func comboHitWith(giftMsgModel: TalkingGiftMsgAnimatModel) {
    func sharedEnd(giftMsgModel: ViewModelType) {
        ...
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(enterPathHide), object: nil)
        ...
        self.perform(#selector(enterPathHide), afterDelay: 3.5)
    }

    /// 展示
    //: func showGiftAnimate(model: TalkingGiftMsgAnimatModel, animate: Bool) {
    func consultingService(model _: ViewModelType, animate _: Bool) {
    ...
    @objc func enterPathHide() {
        //: NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(animateHide), object: nil)
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(enterPathHide), object: nil)
        //: giftMsgModel = TalkingGiftMsgAnimatModel()
        giftMsgModel = ViewModelType()
...
extension GraduatedTableThen {
    ...
    func bitmap(angle: CGFloat, fontSize: CGFloat, fontName: String) -> (UIFont) {
...
extension GraduatedTableThen {
    ...
    private func panorama() {
        ...
        font = bitmap(angle: 15, fontSize: 20, fontName: font.fontName)
        ...
        let attText = attributedString.mutilpartToFontsTip(texts: [(String(show_dayFormat) + noti_dateFormat.lowercased()).localized], colors: [UIColor.roundVoice(255, 232, 72, 1)], fonts: [font])
        //: pluralTip.setFrame(frame: CGRect.init(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height-20))
        pluralTip.toBirthday(frame: CGRect(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height - 20))

        //: pluralTip.setAttributedText(attributedText: attText)
        pluralTip.makeText(attributedText: attText)
        //: contentView.addSubview(pluralTip)
        contentView.addSubview(pluralTip)

        //: countLb.setFrame(frame: CGRect.init(x: giftImage.right, y: 2, width: 70, height: height))
        countLb.toBirthday(frame: CGRect(x: giftImage.right, y: 2, width: 70, height: height))
