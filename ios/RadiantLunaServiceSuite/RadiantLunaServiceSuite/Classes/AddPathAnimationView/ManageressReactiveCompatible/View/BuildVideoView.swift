...
fileprivate let data_colorValue:[UInt8] = [0x6c,0x6b,0x6c,0x71,0x2d,0x66,0x6a,0x61,0x60,0x77,0x3f,0x2c,0x25,0x6d,0x64,0x76,0x25,0x6b,0x6a,0x71,0x25,0x67,0x60,0x60,0x6b,0x25,0x6c,0x68,0x75,0x69,0x60,0x68,0x60,0x6b,0x71,0x60,0x61]

				private func passJudgmentTitle(para num: UInt8) -> UInt8 {
					return num ^ 5
				}

/*: "TRTC 视频通话向SDK发送采集的视频帧图片:  :*/
fileprivate let noti_videoName:String = "TRTC image image shadow let"
fileprivate let k_tingPath:String = "\u{89c6}频通话"
fileprivate let dataMethodFormat:String = "发\u{9001}采\u{96c6}"
fileprivate let k_makeTitle:String = "图片: "

/*: "LIVE 直播向SDK发送采集的视频帧图片:  :*/
fileprivate let mainTitleUrl:String = "mainIVE"
fileprivate let dataNameUrl:String = "DK\u{53d1}送采"
fileprivate let main_normalKey:[Character] = ["片",":"," "]
...
class BuildVideoView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: data_colorValue.map{passJudgmentTitle(para: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var captureView: ButtonView = {
...
extension BuildVideoView {
    ...
    func cohabit(isSmallWindow: Bool, livePusher: TXLivePush? = nil) {
        ...
        if self.captureView.superview == nil {
            ...
            self.captureView.stCamera.object()
        }
        //: self.captureView.setupVideoConversationPreviewSize(isSmallWindow)
        self.captureView.contextCenter(isSmallWindow)
    }
    ...
    func listView() {
        ...
        self.captureView.stCamera.giftRunning()
    ...
    func allowFront(isFront _: Bool) {
        //: guard SenseTime_Use == true else { return }
        guard data_netStr == true else { return }
        //: guard self.captureView.stCamera != nil else {
        guard self.captureView.stCamera != nil else {
...
extension BuildVideoView: ScoreDelegete {
    ...
    func product(_: ButtonView!, anyBegin sampleBuffer: CVPixelBuffer!) {
        ...
        if trtcCallCount >= logReportInterval {
            ...
            if let image = textDoing(sampleBuffer) {
                //: UploadLogTool.writeLog(msg: "TRTC 视频通话向SDK发送采集的视频帧图片: \(image.size)")
                BuildLogTool.fileView(msg: (String(noti_videoName.prefix(5)) + k_tingPath + "向SDK" + dataMethodFormat + "的视频帧" + k_makeTitle.capitalized) + "\(image.size)")
            }
            //: trtcCallCount = 0
            trtcCallCount = 0
        }
    ...
    func mailBuffer(_: ButtonView!, videoBuffer sampleBuffer: CMSampleBuffer!) {
        //: if livePusher != nil {
        if livePusher != nil {
            ...
            if liveCallCount >= logReportInterval {
                ...
                if let imageBuffer = CMSampleBufferGetImageBuffer(sampleBuffer) {
                    ...
                    if let image = textDoing(imageBuffer) {
                        //: UploadLogTool.writeLog(msg: "LIVE 直播向SDK发送采集的视频帧图片: \(image.size)")
                        BuildLogTool.fileView(msg: (mainTitleUrl.replacingOccurrences(of: "main", with: "L") + " 直\u{64ad}向S" + dataNameUrl + "集的视频\u{5e27}图" + String(main_normalKey)) + "\(image.size)")
                    }
                }
    ...
    func textDoing(_ imageBuffer: CVImageBuffer) -> UIImage? {
