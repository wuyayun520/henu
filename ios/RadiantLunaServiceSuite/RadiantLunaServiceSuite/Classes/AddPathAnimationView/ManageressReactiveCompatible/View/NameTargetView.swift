...
fileprivate let dataLayerFormat:[UInt8] = [0xdb,0xdc,0xdb,0xc6,0x9a,0xd1,0xdd,0xd6,0xd7,0xc0,0x88,0x9b,0x92,0xda,0xd3,0xc1,0x92,0xdc,0xdd,0xc6,0x92,0xd0,0xd7,0xd7,0xdc,0x92,0xdb,0xdf,0xc2,0xde,0xd7,0xdf,0xd7,0xdc,0xc6,0xd7,0xd6]

				private func inkingPad(model num: UInt8) -> UInt8 {
					return num ^ 178
				}
...
class NameTargetView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataLayerFormat.map{inkingPad(model: $0)}, encoding: .utf8)!)
    }
    ...
    override func willMove(toSuperview _: UIView?) {
        ...
        GiftCommentSocketDelegate.shared.containerView?.isHidden = false
    }
    ...
    func mp() {
        ...
        UIView.animate(withDuration: 0.3, delay: 2, options: .curveEaseOut) {
            ...
            let x = (self.center.x <= main_colorData * 0.5) ? -self.width : main_colorData + self.width
            ...
            GiftCommentSocketDelegate.shared.containerView?.isHidden = true
        }
    }
}
...
extension NameTargetView {
    /// 添加点击手势
    //: @discardableResult
    @discardableResult
    //: func addTapGesture() -> UITapGestureRecognizer {
    func tapCurrent() -> UITapGestureRecognizer {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(self.tapGestureEvent(sender:)))
        let tap = UITapGestureRecognizer(target: self, action: #selector(self.contentSender(sender:)))
    ...
    func material() {
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(self.occupy(sender:)))
        //: let tap = self.addTapGesture()
        let tap = self.tapCurrent()
    ...
    @objc func occupy(sender: UIPanGestureRecognizer) {
        ...
        centerX = min(main_colorData - self.width * 0.5, centerX)
        //: centerY = max(self.height*0.5, centerY)
        centerY = max(self.height * 0.5, centerY)
        //: centerY = min(ScreenHeight-self.height*0.5, centerY)
        centerY = min(constRecordServerKey - self.height * 0.5, centerY)
        ...
        if sender.state == .ended {
            ...
            var endCenterX = main_colorData - self.width * 0.5
            //: var recordX = ScreenWidth
            var recordX = main_colorData
            //: if centerX <= ScreenWidth*0.5 {
            if centerX <= main_colorData * 0.5 {
            ...
            GiftCommentSocketDelegate.shared.videoMiniPosition = CGPoint(x: recordX, y: recordY)
        }
    }

    /// 点击手势
    //: @objc private func tapGestureEvent(sender: UIPanGestureRecognizer) {
    @objc private func contentSender(sender _: UIPanGestureRecognizer) {
