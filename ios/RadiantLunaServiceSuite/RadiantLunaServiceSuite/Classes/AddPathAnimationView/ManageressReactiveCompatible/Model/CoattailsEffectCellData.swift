...
/*: "#BAFBFF" :*/
fileprivate let dataButtonStr:String = "#BAFBFFview model"
...
class CoattailsEffectCellData: NSObject {
    ...
    var model = ConsumerMeasurable()

    //: func caculateMsgHeight(model: TalkingVideoCallDanmuModel) -> TalkingVideoCallDanmuModel {
    func bindModel(model: ConsumerMeasurable) -> ConsumerMeasurable {
        //: self.model = model
        self.model = model
        /// 普通文本消息
    ...
    func nowModel(model: ConsumerMeasurable) -> ConsumerMeasurable {
        //: self.model = model
        self.model = model
        /// 普通文本消息
        //: if self.model.messageType == 1 {
    ...
    func textComment() {
        ...
        let name = share(text: firstStr, font: UIFont.font(fontSize: 14), color: UIColor(hex: (String(dataButtonStr.prefix(7))))!)
        // 内容
        //: let content = getAttributed(text: self.model.content, font: UIFont.pingfangMediumFont(fontSize: 14), color: UIColor.white)
        let content = share(text: self.model.content, font: UIFont.font(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        ...
        titleMissiveTapBigAttrib(attribText: self.model.msgAttribText)
    }
    ...
    func vocalisation() {
        ...
        let content = share(text: self.model.transContent, font: UIFont.font(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        //: name.yy_paragraphStyle = paraStyle
        name.yy_paragraphStyle = paraStyle
    ...
    func viewStyle() -> NSMutableParagraphStyle {
    ...
    func share(text: String, font: UIFont, color: UIColor) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = viewStyle()
        //: return attribute
        return attribute
    }
    ...
    func titleMissiveTapBigAttrib(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = VideoMsgTableViewWidth - 36
        let maxWidth = showFormalFormat - 36
