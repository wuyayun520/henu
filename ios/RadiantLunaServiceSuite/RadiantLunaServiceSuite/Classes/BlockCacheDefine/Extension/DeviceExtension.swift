...
/*: "RSA加密失败，str:  :*/
fileprivate let data_makeTitle:String = "RSA加shared guard drop for"
fileprivate let kStatusUrl:String = "\u{5bc6}\u{5931}败，s"

/*: "weixin://" :*/
fileprivate let k_titleFrameMessage:String = "W"
fileprivate let const_errorData:String = "eixin://block title direction guide index"

/*: "dingtalk://" :*/
fileprivate let userSuccessPriceName:[Character] = ["d","i","n","g","t","a","l","k",":","/","/"]

/*: "lark://" :*/
fileprivate let noti_segmentKey:String = "lark:/model information"
fileprivate let noti_arrayName:String = "live"

/*: "wxwork://" :*/
fileprivate let noti_dataTitle:String = "WXWO"

/*: "deviceId" :*/
fileprivate let app_failName:[UInt8] = [0xdc,0xdd,0xce,0xd1,0xdb,0xdd,0xf1,0xdc]

				private func imagePic(count num: UInt8) -> UInt8 {
					return num ^ 184
				}

/*: "weixin" :*/
fileprivate let showModelToImageText:String = "weinputxinputn"

/*: "wxwork" :*/
fileprivate let user_blockMessage:String = "WXWOR"
fileprivate let app_limitNamePath:[Character] = ["k"]

/*: "dingtalk" :*/
fileprivate let userBetweenUrl:[Character] = ["d","i","n","g","t","a","l","k"]

/*: "lark" :*/
fileprivate let const_equalValue:String = "luprk"
...
extension UIDevice {
    ...
    class func colorBlock(IdentifierBlock: @escaping (_ str: String) -> Void) {
        ...
        if #available(iOS 14.0, *) {
            ...
            ATTrackingManager.requestTrackingAuthorization(completionHandler: { status in
                ...
                if status == .authorized {
                    ...
                    IdentifierBlock(UIDevice.untilRaw())
                    //: } else {
                } else {
                    //: IdentifierBlock("")
                    IdentifierBlock("")
    ...
    class func marginOfSafety() -> ([String]) {
...
//            let mnc = carrier?.mobileNetworkCode  //移动网络代码
    ...
    static func kill(str: String) -> String {
        //: var encryptedStr = ""
        var encryptedStr = ""
        //: do {
        do {
            ...
            printLog(message: (String(data_makeTitle.prefix(4)) + kStatusUrl + "tr: ") + "\(str).")
        }
        //: return encryptedStr
        return encryptedStr
    }
...
public extension UIDevice {
    ...
    static var isOpenWX: Bool {
        ...
        let appSchema = (k_titleFrameMessage.lowercased() + String(const_errorData.prefix(8)))
    ...
    static var isOpenDD: Bool {
        ...
        let appSchema = (String(userSuccessPriceName))
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var isOpenFS: Bool {
        //: let appSchema = "lark://"
        let appSchema = (String(noti_segmentKey.prefix(6)) + noti_arrayName.replacingOccurrences(of: "live", with: "/"))
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var isOpenWXWork: Bool {
        ...
        let appSchema = (noti_dataTitle.lowercased() + "rk://")
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var deviceInfoDic: [String: Any] {
        ...
        let usedVpn = QuantityRequestAddrTool.utilisedProxy() || QuantityRequestAddrTool.rowColor()
        ...
        if let paramsStr = String(data: data!, encoding: String.Encoding.utf8) {
            ...
            let deviceIdStr = kill(str: adid)
            //: return ["deviceId": deviceIdStr,
            return [String(bytes: app_failName.map{imagePic(count: $0)}, encoding: .utf8)!: deviceIdStr,
                    //: "p0": paramsStr]
                    "p0": paramsStr]
        }
        //: return [:]
    ...
    static var getApps: String? {
        ...
        if UIDevice.isOpenWX {
            ...
            appsArr.append((showModelToImageText.replacingOccurrences(of: "input", with: "i")))
        }
        //: if UIDevice.isOpenWXWork {
        if UIDevice.isOpenWXWork {
            //: appsArr.append("wxwork")
            appsArr.append((user_blockMessage.lowercased() + String(app_limitNamePath)))
        }
        //: if UIDevice.isOpenDD {
        if UIDevice.isOpenDD {
            //: appsArr.append("dingtalk")
            appsArr.append((String(userBetweenUrl)))
        }
        //: if UIDevice.isOpenFS {
        if UIDevice.isOpenFS {
            //: appsArr.append("lark")
            appsArr.append((const_equalValue.replacingOccurrences(of: "up", with: "a")))
        }
        //: if appsArr.count > 0 {
        if appsArr.count > 0 {
