...
- (NSData *)giftWith {
    ...
    UIImage *image = self.timelineAnimalComparable;
...
- (UIImage *)filter {
    ...
    return [UIImage imageWithData:[self giftWith]];
}


//: - (UIImage *)wcSessionCompress {
- (UIImage *)volitionCompress {
    //: return [self wcCompres:YES];
    return [self stop:YES];
}

//: - (UIImage *)wcTimelineCompress {
- (UIImage *)timelineAnimalComparable {
...
- (UIImage *)stop:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self visualization:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self noneMentalRepresentation:size];
...
- (CGSize)visualization:(Boolean)isSession {
...
+ (NSData *)video:(UIImage *)image attractor:(NSInteger)fileSize{
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG sendContent:scalesize statusScale:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG modelRange:(fileSize*1024)];
    }
...
- (UIImage*)gift:(CGSize)targetSize{
...
- (UIImage *)sendContent:(CGSize)targetSize statusScale:(BOOL)isScale{
...
- (NSData *)modelRange:(NSInteger)fileSize{
...
- (UIImage *)noneMentalRepresentation:(CGSize)newSize {
