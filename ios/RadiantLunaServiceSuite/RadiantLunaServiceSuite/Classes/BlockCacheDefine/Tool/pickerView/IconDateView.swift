...
/*: "Select a date" :*/
fileprivate let k_lineMessage:String = "Selecview else"
fileprivate let userOnPath:[Character] = ["t"," ","a"," ","d","a","t","e"]
...
protocol GestureOptionViewDelegate: NSObjectProtocol {
...
class IconDateView: NamePickView, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    weak var delegate: GestureOptionViewDelegate?
    ...
    override func pickOut() {
        ...
        titleString = (String(k_lineMessage.prefix(5)) + String(userOnPath)).localized
        ...
        setWithoutDaytimeTitle(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func setWithoutDaytimeTitle(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func clickSustainButton(button: UIButton) {
        ...
        if let day = selectDay {
            ...
            delegate?.equalDay(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.equalDay(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return confinementMonth(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func confinementMonth(year: Int, month: Int) -> Int {
