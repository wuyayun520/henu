...
fileprivate let noti_lineMessage:String = "btn_memin empty"
fileprivate let appClearKey:String = "k_coinput card for center"

/*: "init(coder:) has not been implemented" :*/
fileprivate let data_kitKey:[UInt8] = [0xb7,0xb0,0xb7,0xaa,0xf6,0xbd,0xb1,0xba,0xbb,0xac,0xe4,0xf7,0xfe,0xb6,0xbf,0xad,0xfe,0xb0,0xb1,0xaa,0xfe,0xbc,0xbb,0xbb,0xb0,0xfe,0xb7,0xb3,0xae,0xb2,0xbb,0xb3,0xbb,0xb0,0xaa,0xbb,0xba]

				private func toModel(ting num: UInt8) -> UInt8 {
					return num ^ 222
				}
...
class CommentAddThen: UIView {
    //: private var alertConfig: ShowAlertConfig
    private var alertConfig: InsertAlertConfig
    ...
    private lazy var leftBtn: UIButton = {
        ...
        leftBtn.layer.borderColor = UIColor.actionLive().cgColor
        //: leftBtn.layer.borderWidth = 1
        leftBtn.layer.borderWidth = 1
        //: leftBtn.addTarget(self, action: #selector(leftClick), for: .touchUpInside)
        leftBtn.addTarget(self, action: #selector(pushClick), for: .touchUpInside)
        //: return leftBtn
        return leftBtn
        //: }()
    }()
    ...
    private lazy var rightBtn: UIButton = {
        ...
        let image = UIImage.bundle(name: alertConfig.rightBackgroundImage).withTintColor(.actionLive())
        //: rightBtn.setBackgroundImage(image, for: .normal)
        rightBtn.setBackgroundImage(image, for: .normal)
        //: rightBtn.addTarget(self, action: #selector(rightClick), for: .touchUpInside)
        rightBtn.addTarget(self, action: #selector(preemption), for: .touchUpInside)
        //: return rightBtn
        return rightBtn
        //: }()
    }()
         ...
         config: InsertAlertConfig)
    {
        ...
        if leftBtnTitle == nil && leftBtnAttributedTitle == nil {
            ...
            alertConfig.rightBackgroundImage = (String(noti_lineMessage.prefix(6)) + "_bac" + String(appClearKey.prefix(4)) + "ntinue")
        }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: data_kitKey.map{toModel(ting: $0)}, encoding: .utf8)!)
    }

    //: func changeRightBtn(title: String, isTouch: Bool) {
    func waterRight(title: String, isTouch: Bool) {
        ...
        if !isTouch {
            ...
            rightBtn.setBackgroundImage(UIImage.bundle(name: alertConfig.rightBackgroundImagefalse), for: .normal)
            //: } else {
        } else {
            //: let image = UIImage.BundleImageNamed(name: alertConfig.rightBackgroundImage).withTintColor(.appThemeColor())
            let image = UIImage.bundle(name: alertConfig.rightBackgroundImage).withTintColor(.actionLive())
