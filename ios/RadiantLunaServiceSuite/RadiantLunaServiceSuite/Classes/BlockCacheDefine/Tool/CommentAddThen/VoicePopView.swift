...
/*: "cellID" :*/
fileprivate let const_valueSpacePath:String = "pic view layercellID"

/*: "init(coder:) has not been implemented" :*/
fileprivate let dataModelUrl:[UInt8] = [0x3,0x4,0x3,0x1e,0x42,0x9,0x5,0xe,0xf,0x18,0x50,0x43,0x4a,0x2,0xb,0x19,0x4a,0x4,0x5,0x1e,0x4a,0x8,0xf,0xf,0x4,0x4a,0x3,0x7,0x1a,0x6,0xf,0x7,0xf,0x4,0x1e,0xf,0xe]

/*: "Video Call" :*/
fileprivate let show_bottomMessage:[Character] = ["V","i","d","e","o"," ","C","a","l"]
fileprivate let noti_imageTitle:String = "mode"

/*: "Video Call (%@ Points/Min)" :*/
fileprivate let main_resultMsg:String = "Videobasic kind"
fileprivate let app_listText:String = "block self let to true(%@ Po"
fileprivate let constStatusData:String = "/Min)total self name color"

/*: "Voice Call" :*/
fileprivate let showToMessage:[Character] = ["V","o","i","c","e"," ","C","a"]
fileprivate let const_sessionFormat:[Character] = ["l","l"]

/*: "Voice Call (%@ Points/Min)" :*/
fileprivate let noti_actionStr:String = "empty name sectionVoice "
fileprivate let show_makePath:[Character] = [" ","P","o"]
fileprivate let notiMonthKey:[Character] = ["i","n","t","s","/","M","i","n",")"]

/*: "#F8F8F8" :*/
fileprivate let show_actualPath:String = "#"
fileprivate let showPreviousUrl:[Character] = ["F","8","F","8","F","8"]

/*: "Cancel" :*/
fileprivate let appFilterKey:String = "size"
fileprivate let notiErrorText:[Character] = ["a","n","c","e","l"]

/*: "#F5F5F5" :*/
fileprivate let main_imageInfoName:[Character] = ["#"]
fileprivate let constApplicationMessage:[Character] = ["F","5","F","5","F","5"]

/*: "icon_lounge" :*/
fileprivate let showCoverData:[Character] = ["i","c","o","n","_","l","o","u","n","g","e"]
...
class VoicePopView: UIView {
    //: var munuBlock: MunuBlock?
    var munuBlock: MunuBlock?

    //: private var type: MunuPopType = .Unknown
    private var type: EffectEditPopType = .Unknown // 当前展示类型
    ...
    var cellTitleColor = UIColor.sendTitle()
    /// cell字体大小
    //: var cellTitleFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
    var cellTitleFont = UIFont.replyName(type: .Regular, fontSize: 16)
    /// 是否有"Cancel"菜单
    //: var hasCancel = true
    var hasCancel = true
    /// cancel字体颜色
    //: var cancelColor = UIColor.appValueDetailColor()
    var cancelColor = UIColor.valueColor()
    /// cancel字体大小
    //: var cancelTitleFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
    var cancelTitleFont = UIFont.replyName(type: .Regular, fontSize: 16)
    /// 标题
    //: var title: String?
    var title: String?
    /// 标题颜色
    //: var titleColor = UIColor.appThemeColor()
    var titleColor = UIColor.actionLive()
    /// 标题字体大小
    //: var titleFont = UIFont.pingfangMediumFont(fontSize: 16)
    var titleFont = UIFont.font(fontSize: 16)

    //: var tableView = UITableView()
    var tableView = UITableView()
    //: let cellID = "cellID"
    let cellID = (String(const_valueSpacePath.suffix(6)))
    ...
    fileprivate var footerViewHight: CGFloat {
        ...
        return hasCancel ? (footerViewLineHight + 50 + noti_saveFormat) : noti_saveFormat
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataModelUrl.map{$0^106}, encoding: .utf8)!)
    }

    //: func initwithList(cellTitleList: [String]!) {
    func managerList(cellTitleList: [String]!) {
        // 初始化
    ...
    func beforeDown(type: EffectEditPopType, videoPrice: String?, voicePrice: String?) {
        //: self.type = type
        self.type = type
        //: if self.type == .Call {
        if self.type == .Call { // 音视频通话
            ...
            if IconContainerAppManager.share.appStatus == MinExpressionConvertible.normal.rawValue {
                ...
                if IconContainerAppManager.share.loginUserMode.sex == ThemeTitleScalarLiteral.male.rawValue {
                    ...
                    let videoAttrStr = String.hail(VideoPrice: Int(videoPrice ?? "0") ?? 0)
                    //: let voiceAttrStr = String.setVoicePriceStr(VoicePrice: Int(voicePrice ?? "0") ?? 0)
                    let voiceAttrStr = String.strengthPrice(VoicePrice: Int(voicePrice ?? "0") ?? 0)
                    ...
                    let attributes = [.foregroundColor: UIColor.actionLive(), .font: UIFont.replyName(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
                    //: let VideoCall = "Video Call".localized
                    let VideoCall = (String(show_bottomMessage) + noti_imageTitle.replacingOccurrences(of: "mode", with: "l")).localized
                    //: let videoStr = "Video Call (%@ Points/Min)".localizedArguments(videoPrice ?? "0")
                    let videoStr = (String(main_resultMsg.prefix(5)) + " Call " + String(app_listText.suffix(6)) + "ints" + String(constStatusData.prefix(5))).cellArguments(videoPrice ?? "0")
                    ...
                    let VoiceCall = (String(showToMessage) + String(const_sessionFormat)).localized
                    //: let voiceStr = "Voice Call (%@ Points/Min)".localizedArguments(voicePrice ?? "0")
                    let voiceStr = (String(noti_actionStr.suffix(6)) + "Call (%@" + String(show_makePath) + String(notiMonthKey)).cellArguments(voicePrice ?? "0")
                ...
                self.cellTitleList = [(String(show_bottomMessage) + noti_imageTitle.replacingOccurrences(of: "mode", with: "l")).localized, (String(showToMessage) + String(const_sessionFormat)).localized]
            }
        }
    ...
    public func ping() {
        ...
        tableView = UITableView(frame: CGRect(x: 0, y: constRecordServerKey - tableViewHight, width: main_colorData, height: tableViewHight), style: .plain)
        ...
        tableView.register(DeedView.self, forCellReuseIdentifier: cellID)
        ...
        if title == nil {
            ...
            tableView.meanSunRadii(width: main_colorData,
...
extension VoicePopView: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var cell = tableView.dequeueReusableCell(withIdentifier: cellID) as? DeedView
        //: if cell == nil {
        if cell == nil {
            //: cell = TitleCell(style: .default, reuseIdentifier: cellID)
            cell = DeedView(style: .default, reuseIdentifier: cellID)
        }

        //: if IconContainerAppManager.share.loginUserMode.sex == Gender.male.rawValue,
        if IconContainerAppManager.share.loginUserMode.sex == ThemeTitleScalarLiteral.male.rawValue,
           //: IconContainerAppManager.share.loginUserMode.loungePlus == true,
           IconContainerAppManager.share.loginUserMode.loungePlus == true,
           //: type == .Call {
           type == .Call
        {
        ...
        cell!.titleLab.font = cellTitleFont
        //: if self.type == .Call && IconContainerAppManager.share.appStatus == AppSkinStatus.normal.rawValue {
        if self.type == .Call && IconContainerAppManager.share.appStatus == MinExpressionConvertible.normal.rawValue {
    ...
    func tableView(_: UITableView, viewForFooterInSection _: Int) -> UIView? {
        ...
        let footerView = UIView(frame: CGRect(x: 0, y: 0, width: main_colorData, height: footerViewHight))
        //: let lineView = UIView(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: footerViewLineHight))
        let lineView = UIView(frame: CGRect(x: 0, y: 0, width: main_colorData, height: footerViewLineHight))
        //: lineView.backgroundColor = UIColor(hex: "#F8F8F8")
        lineView.backgroundColor = UIColor(hex: (show_actualPath.capitalized + String(showPreviousUrl)))
        ...
        btn.frame = CGRect(x: 0, y: footerViewLineHight, width: main_colorData, height: 50)
        //: footerView.addSubview(btn)
        footerView.addSubview(btn)
        //: btn.setTitle("Cancel".localized, for: .normal)
        btn.setTitle((appFilterKey.replacingOccurrences(of: "size", with: "C") + String(notiErrorText)).localized, for: .normal)
    ...
    func tableView(_: UITableView, viewForHeaderInSection _: Int) -> UIView? {
        ...
        let headerView = UIView(frame: CGRect(x: 0, y: 0, width: main_colorData, height: headerViewHight))

        //: let titleLab = UILabel(frame: CGRect(x: 15, y: 0, width: ScreenWidth-30, height: headerViewHight))
        let titleLab = UILabel(frame: CGRect(x: 15, y: 0, width: main_colorData - 30, height: headerViewHight))
        ...
        let lineView = UIView(frame: CGRect(x: 0, y: headerViewHight - 1, width: main_colorData, height: 1))
        //: lineView.backgroundColor = UIColor.init(hex: "#F5F5F5")
        lineView.backgroundColor = UIColor(hex: (String(main_imageInfoName) + String(constApplicationMessage)))
...
class DeedView: UITableViewCell {
    ...
    lazy var vipIcon: UIImageView = {
        ...
        img.image = UIImage.bundle(name: (String(showCoverData)))
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    lazy var lineLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(main_imageInfoName) + String(constApplicationMessage)))
        //: return lab
        return lab
        //: }()
    }()
