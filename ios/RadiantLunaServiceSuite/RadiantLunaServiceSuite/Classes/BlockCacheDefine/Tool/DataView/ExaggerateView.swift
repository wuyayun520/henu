...
fileprivate let show_resultSmallMomentId:[UInt8] = [0x45,0x4a,0x45,0x50,0x4,0x3f,0x4b,0x40,0x41,0x4e,0x16,0x5,0xfc,0x44,0x3d,0x4f,0xfc,0x4a,0x4b,0x50,0xfc,0x3e,0x41,0x41,0x4a,0xfc,0x45,0x49,0x4c,0x48,0x41,0x49,0x41,0x4a,0x50,0x41,0x40]

				fileprivate func pushBackground(position num: UInt8) -> UInt8 {
...
typealias SlidingHandler = (_ direction: VideoEqualSlideDirection) -> Void
...
class ExaggerateView: UIView {
    ...
    var cropStart: CGFloat {
        ...
        let offset = overlay.startOffset - (user_barMsg - self.contentView.contentOffset.x)
    ...
    init(frame: CGRect, url: URL, imgWidth: CGFloat, maxDuration: CGFloat, minVideoDuration: CGFloat) {
        ...
        //: setupSubviews()
        chemicalElementSubviews()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: show_resultSmallMomentId.map{pushBackground(position: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.timePoint(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: DataView = {
        ...
        let overlay = DataView(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension ExaggerateView {
    ...
    private func timePoint(point: CGFloat) -> UIImage {
...
extension ExaggerateView: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = user_barMsg - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_: UIScrollView) {
        ...
        self.overlay.commentTo(isShow: false)
    }
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        self.overlay.commentTo(isShow: true)
    }
...
extension ExaggerateView {
    ...
    private func chemicalElementSubviews() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(user_barMsg)
