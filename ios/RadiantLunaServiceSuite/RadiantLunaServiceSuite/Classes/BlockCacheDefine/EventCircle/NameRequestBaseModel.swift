...
/*: "http://stat. :*/
fileprivate let k_userBurnName:[Character] = ["h","t","t","p",":","/","/","s","t","a","t","."]

/*: "http://" :*/
fileprivate let notiPresentationFormatPath:String = "http://return fatal self"

/*: "https://" :*/
fileprivate let mainCellFormat:[Character] = ["h","t","t","p","s",":","/","/"]

/*: "http://m. :*/
fileprivate let data_bringFormat:String = "back equal casehttp://"
fileprivate let noti_toolTitle:String = "m.error unknown up make"

/*: "http:// :*/
fileprivate let notiAddData:[Character] = ["h","t","t","p",":","/"]
fileprivate let constAppStr:String = "/"

/*: "1990-01-01" :*/
fileprivate let notiColorUpId:[Character] = ["1","9","9","0","-","0","1"]
fileprivate let userLeadingTakeId:String = "-01"

/*: "MM-dd-yyyy" :*/
fileprivate let constMakeId:String = "show"
fileprivate let const_formatMenuName:String = "top excess self frameM-dd-"
...
class SaveThen: NSObject {
    ...
    var requestType: TalkingThemeVariantRequestType?
    ...
    override init() {
        ...
        self.requestServer = QuantityRequestAddrTool.share.serverUrlStr
...
struct SizeBaseResponse: HandyJSON {
    //: var errno: Int!
...
public struct ActualErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
struct InsertConfigTransformable: HandyJSON {
    ...
    var CSConfig = GestureMeasurable() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (String(k_userBurnName)) + "\(app_liveUrl).com"
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(notiPresentationFormatPath.prefix(7)))) && !statDomain.hasPrefix((String(mainCellFormat))) {
                //: return "http://".appending(statDomain)
                return (String(notiPresentationFormatPath.prefix(7))).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (String(data_bringFormat.suffix(7)) + String(noti_toolTitle.prefix(2))) + "\(app_liveUrl).com"
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(notiAddData) + constAppStr.capitalized) + "\(urlDomain)"
        }
    }
...
public class BrisanceReactiveCompatible: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [CellHandyJSON] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [CellHandyJSON] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [CellHandyJSON] = [] // 语音收费设置
    ...
    var vipSetting = [FuturismHandyJSON]() // vip皮肤
    //: var gameShowBit = 0
    ...
    var tabPageBanner = [SprechgesangHandyJSON]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = CellSourceHandyJSON() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class ImageThen: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [EditTagModel]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [EditTagModel]?
    //: var partner: String?
    var partner: String?
    //: var full: Int?
    var full: Int?
    ...
    var gallery: [EyeglassesGalleryModel]?
    //: var status: Int = -1
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.getBack(self.birthday ?? (String(notiColorUpId) + userLeadingTakeId.capitalized), dateFormat: (constMakeId.replacingOccurrences(of: "show", with: "M") + String(const_formatMenuName.suffix(5)) + "yyyy"))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.totaleractionComponents().year else { return "0" }
...
struct EyeglassesGalleryModel: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct EditTagModel: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func afterPush(compareTo: EditTagModel) -> Bool {
        //: return
        return
...
struct CatScanMeasurable: HandyJSON {
    //: var aboutMe: Array<UserTagTypeModel>!
    var aboutMe: [BlockModelType]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [BlockModelType]!
}

//: struct UserTagTypeModel: HandyJSON {
struct BlockModelType: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
    ...
    var list: [EditTagModel]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct GestureMeasurable: HandyJSON {
...
struct FuturismHandyJSON: HandyJSON {
...
struct CellSourceHandyJSON: HandyJSON {
    //: var sendImg = 10
    var sendImg = 10 // 发送图片
    //: var sendVideo = 10
