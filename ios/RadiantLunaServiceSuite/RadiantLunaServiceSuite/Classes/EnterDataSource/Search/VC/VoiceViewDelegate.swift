...
/*: "Search" :*/
fileprivate let mainCountStr:String = "premium"
fileprivate let kBottomName:String = "EARCH"

/*: "#F5F5F5" :*/
fileprivate let dataBackgroundEachViewTitle:String = "#Findex stage lab bar view"
fileprivate let kAppName:String = "5F5F5extension true"

/*: "Search by UID" :*/
fileprivate let main_shareStr:[Character] = ["S","e","a","r","c","h"," ","b","y"," "]
fileprivate let dataManagerKey:String = "UIfollowing"

/*: "icon_search_sea" :*/
fileprivate let showLabViewRecordData:String = "ICON"
fileprivate let constModeName:String = "ch_seaview height self"

/*: "Can’t find~" :*/
fileprivate let data_videoValue:String = "Catitle make var mode"
fileprivate let appFillKey:String = "n\u{2019}t f"

/*: "icon_kong_kong_default" :*/
fileprivate let const_curId:String = "distribution gesture videoicon_"
fileprivate let notiIndexStr:[Character] = ["o","n","g","_","d","e","f","a","u","l","t"]

/*: "Please enter the query UID" :*/
fileprivate let main_requestData:String = "image dataPlease "
fileprivate let k_equalData:String = "to array view nameenter t"
fileprivate let mainShowText:String = "he quevoice text or in"
...
class VoiceViewDelegate: LightThen {
    ...
    private lazy var searchBtn: UIButton = {
        ...
        btn.setTitle((mainCountStr.replacingOccurrences(of: "premium", with: "S") + kBottomName.lowercased()).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.smallSize(colors: UIColor.equalAction(), size: CGSize(width: 72, height: 32)), for: .normal)
        //: btn.layer.cornerRadius = 16
        btn.layer.cornerRadius = 16
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(searchButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(femaleClick), for: .touchUpInside)
        //: btn.alpha = 0.5
        btn.alpha = 0.5
    ...
    private lazy var searchTF: UITextField = {
        ...
        tf.backgroundColor = UIColor(hex: (String(dataBackgroundEachViewTitle.prefix(2)) + String(kAppName.prefix(5))))
        ...
        tf.placeholder = (String(main_shareStr) + dataManagerKey.replacingOccurrences(of: "following", with: "D")).localized
    ...
    private lazy var unlessEditingView: UIView = {
        ...
        let image = UIImage.bundle(name: (showLabViewRecordData.lowercased() + "_sear" + String(constModeName.prefix(6))))
    ...
    private lazy var tableView: UITableView = {
        ...
        tabV.register(MomentItemCell.self, forCellReuseIdentifier: MomentItemCell.className())
    ...
    private lazy var emptyV: EmptyView = {
        ...
        style.TipsTitle = (String(data_videoValue.prefix(2)) + appFillKey + "ind~").localized
        //: style.TipsIcon = "icon_kong_kong_default"
        style.TipsIcon = (String(const_curId.suffix(5)) + "kong_k" + String(notiIndexStr))
        ...
        let v = EmptyView(frame: CGRect(x: 0, y: 14 + user_ofMsg, width: main_colorData, height: constRecordServerKey - (14 + user_ofMsg + data_halfMsg)), style: style)
    ...
    private lazy var reqManager: SpectaclesSearchManager = //: return SpectaclesSearchManager()
        .init()
    //: }()
}
...
extension VoiceViewDelegate {
    ...
    private func examineProgress() {
        //: self.view.endEditing(true)
        self.view.endEditing(true)
        //: guard searchTF.text?.count ?? 0 > 0 else {
        guard searchTF.text?.count ?? 0 > 0 else {
            //: self.func__showStatusBarErrorMsg(showMsg: "Please enter the query UID".localized)
            self.explain(showMsg: (String(main_requestData.suffix(7)) + String(k_equalData.suffix(7)) + String(mainShowText.prefix(6)) + "ry UID").localized)
            //: return
            return
        }
        ...
        reqManager.singleAcross(uid: searchTF.text ?? "") { [weak self] _, _, _ in
            //: ProgressHUD.dismiss()
...
extension VoiceViewDelegate: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: MomentItemCell.className(), for: indexPath) as! MomentItemCell
        //: let model = reqManager.searchListDataArr[indexPath.row]
        let model = reqManager.searchListDataArr[indexPath.row]
        //: cell.refreshCell(model: model)
        cell.textTheoryModel(model: model)
...
extension VoiceViewDelegate: UITextFieldDelegate {
...
extension VoiceViewDelegate: JXPagingViewListViewDelegate {
...
extension VoiceViewDelegate {
    ...
    private func playerDismissPath() {
    ...
    private func familyNameQuantityeraction() {
        // 监听搜索框文本长度
