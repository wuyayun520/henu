...
fileprivate let data_sizeName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "bg_medals_podium" :*/
fileprivate let showUserStatusMessage:String = "bg_mtouch size model section"
fileprivate let userPathUrl:String = "dititlem"

/*: "icon_rank_first_bg" :*/
fileprivate let app_genderMessage:String = "icon_rsource leading at"
fileprivate let appResultTitle:String = "value reply icon collection sizeank_fi"

/*: "icon_rank_first" :*/
fileprivate let mainSomeoneValue:[Character] = ["i","c","o","n","_","r","a","n","k","_"]
fileprivate let app_equalMessage:[Character] = ["f","i","r","s","t"]

/*: "icon_ammount_points" :*/
fileprivate let app_selectedUrl:[Character] = ["i","c","o","n","_","a","m","m"]
fileprivate let data_giftMessage:[Character] = ["o","u","n","t","_","p"]
fileprivate let constLastPath:[Character] = ["o","i","n","t","s"]

/*: "icon_lounge" :*/
fileprivate let appExplainData:String = "iccontentn"
fileprivate let k_modelPath:String = "need"

/*: "icon_rank_coin" :*/
fileprivate let data_resultFormat:String = "IC"
fileprivate let const_numberUrl:String = "on_rback cell end"

/*: "icon_rank_coin2" :*/
fileprivate let mainTopMsg:[Character] = ["i","c","o","n","_","r","a","n","k"]
fileprivate let appBindKey:String = "to let image_coin2"

/*: "icon_rank_second" :*/
fileprivate let const_valuePath:String = "let viewicon_"
fileprivate let app_handleFemaleBigFormat:String = "_secondvar target"

/*: "icon_rank_third" :*/
fileprivate let data_styleAddViewTitle:String = "icon_rtext model stage string"
fileprivate let data_fatalTitle:String = "equaln"
fileprivate let appContentUrl:String = "cell startk_third"
...
class GestureHeaderView: UIView {
    //: var  index = 0
    var index = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: data_sizeName.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var medalsView = UIImageView().then {
        ...
        $0.image = UIImage.bundle(name: (String(showUserStatusMessage.prefix(4)) + "edals_po" + userPathUrl.replacingOccurrences(of: "title", with: "u")))
    }

    //: lazy var firstView = SocialRankHeaderItemView.init(frame: .zero, rankType: .firstRankType).then {
    lazy var firstView = LineItemView(frame: .zero, rankType: .firstRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 0
        $0.tag = 0
        //: let tap = UITapGestureRecognizer.init(target: self, action: #selector(registTapAction))
        let tap = UITapGestureRecognizer(target: self, action: #selector(userQuery))
        //: $0.addGestureRecognizer(tap)
        $0.addGestureRecognizer(tap)
    }

    //: lazy var secondView = SocialRankHeaderItemView.init(frame: .zero, rankType: .secondRankType).then {
    lazy var secondView = LineItemView(frame: .zero, rankType: .secondRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 1
        $0.tag = 1
    ...
    lazy var thirdView = LineItemView(frame: .zero, rankType: .thirdRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 2
        $0.tag = 2
...
extension GestureHeaderView {
    ...
    private func viewSmooth() {
        //: let firstWidth = (ScreenWidth - actualWidth(w: 68)) / 3 + actualWidth(w: 10)
        let firstWidth = (main_colorData - actualWidth(w: 68)) / 3 + actualWidth(w: 10)
        //: let itemWidth = (ScreenWidth - actualWidth(w: 68)) / 3 - actualWidth(w: 10)
        let itemWidth = (main_colorData - actualWidth(w: 68)) / 3 - actualWidth(w: 10)
...
class LineItemView: UIView {
    ...
    public init(frame: CGRect, rankType: InventoryItemCell) {
        //: super.init(frame: frame)
        super.init(frame: frame)
        //: self.setupItemViews(type: rankType)
        self.nookAndCranny(type: rankType)
    }
    ...
    lazy var headPicBgView = UIImageView().then {
        ...
        $0.image = UIImage.bundle(name: (String(app_genderMessage.prefix(6)) + String(appResultTitle.suffix(6)) + "rst_bg"))
    }
    ...
    lazy var crownImgView = UIImageView().then {
        ...
        $0.image = UIImage.bundle(name: (String(mainSomeoneValue) + String(app_equalMessage)))
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .replyName(type: .Medium, fontSize: 18)
        //: $0.textAlignment = .center
        $0.textAlignment = .center
    }

    //: lazy var coinButton = TalkingButton().then {
    lazy var coinButton = BlockShouldShirtButton().then {
        //: $0.isUserInteractionEnabled = false
        $0.isUserInteractionEnabled = false
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_ammount_points"), for: .normal)
        $0.setImage(UIImage.bundle(name: (String(app_selectedUrl) + String(data_giftMessage) + String(constLastPath))), for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 13)
        $0.titleLabel?.font = .replyName(type: .Regular, fontSize: 13)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.bundle(name: (appExplainData.replacingOccurrences(of: "content", with: "o") + "_loung" + k_modelPath.replacingOccurrences(of: "need", with: "e")))
    }
}
...
extension LineItemView {
    ...
    func restoreName(model: EqualModelType?, type: Int) {
        ...
        headPic.tillFirstBar(urlStr: model!.headPic, placeImg: UIImage.colorSex(sex: model!.sex))
        ...
        if type == 0 {
            ...
            coinButton.setImage(UIImage.bundle(name: (data_resultFormat.lowercased() + String(const_numberUrl.prefix(4)) + "ank_coin")), for: .normal)
            //: } else {
        } else {
            //: coinButton.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin2"), for: .normal)
            coinButton.setImage(UIImage.bundle(name: (String(mainTopMsg) + String(appBindKey.suffix(6)))), for: .normal)
        }
    }
}
...
extension LineItemView {
    ...
    func nookAndCranny(type: InventoryItemCell) {
...
//        var topCrownMargin = actualHeight(h: 4)
        ...
        case .firstRankType:
            ...
            crownImgView.image = UIImage.bundle(name: (String(mainSomeoneValue) + String(app_equalMessage)))
            //: headPic.layer.borderColor = UIColor.RGBA(255, 224, 58, 1).cgColor
            headPic.layer.borderColor = UIColor.roundVoice(255, 224, 58, 1).cgColor
...
//            topCrownMargin = actualHeight(h: 10)
        ...
        case .secondRankType:
            ...
            crownImgView.image = UIImage.bundle(name: (String(const_valuePath.suffix(5)) + "rank" + String(app_handleFemaleBigFormat.prefix(7))))
            //: headPic.layer.borderColor = UIColor.RGBA(182, 199, 254, 1).cgColor
            headPic.layer.borderColor = UIColor.roundVoice(182, 199, 254, 1).cgColor
        ...
        case .thirdRankType:
            ...
            crownImgView.image = UIImage.bundle(name: (String(data_styleAddViewTitle.prefix(6)) + data_fatalTitle.replacingOccurrences(of: "equal", with: "a") + String(appContentUrl.suffix(7))))
            //: headPic.layer.borderColor = UIColor.RGBA(251, 166, 109, 1).cgColor
            headPic.layer.borderColor = UIColor.roundVoice(251, 166, 109, 1).cgColor
            //: headPic.layer.cornerRadius = actualWidth(w: 31)
            headPic.layer.cornerRadius = actualWidth(w: 31)
            //: break
        }
