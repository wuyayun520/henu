...
fileprivate let main_commitMessage:[UInt8] = [0x45,0x42,0x45,0x58,0x4,0x4f,0x43,0x48,0x49,0x5e,0x16,0x5,0xc,0x44,0x4d,0x5f,0xc,0x42,0x43,0x58,0xc,0x4e,0x49,0x49,0x42,0xc,0x45,0x41,0x5c,0x40,0x49,0x41,0x49,0x42,0x58,0x49,0x48]

/*: "#30D00B" :*/
fileprivate let userMakeFormat:String = "#30D0fill self raw return var"
fileprivate let k_inputPath:[Character] = ["0","B"]

/*: "icon_receivedcalls" :*/
fileprivate let notiAbsPath:String = "icon_rif color make var disappear"
fileprivate let showAttachStr:[Character] = ["e","c","e","i","v","e","d","c","a","l","l","s"]

/*: "Received calls" :*/
fileprivate let const_itsData:String = "Receiheight beauty content"
fileprivate let app_numberSizeMessage:[Character] = ["l","l","s"]

/*: "icon_dialedcalls" :*/
fileprivate let k_userTitle:[Character] = ["i","c","o","n"]
fileprivate let userMatchShareValue:String = "_diasucceed var shared raw"

/*: "Dialed calls" :*/
fileprivate let notiSizeStr:String = "main equal filterDia"
fileprivate let dataMakeMsg:String = "if equalled "
...
class OrdinalThen: UITableViewCell {
    //: var currenModel = TalkingVideoCallRecordModel()
    var currenModel = RecordMeasurable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_commitMessage.map{$0^44}, encoding: .utf8)!)
    }
    ...
    lazy var iconBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(weltClick), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var durationLB: UILabel = {
        ...
        label.font = .replyName(type: .Medium, fontSize: 10)
        //: label.textColor = .appValueDetailColor()
        label.textColor = .valueColor()
    ...
    lazy var onlineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (String(userMakeFormat.prefix(5)) + String(k_inputPath)))
...
extension OrdinalThen {
    ...
    func tv(model: RecordMeasurable) {
        ...
        if !model.headPicFrame.isEmptyString {
            ...
            iconBorderBtn.used(urlStr: model.headPicFrame)
            //: } else {
        } else {
        ...
        if model.callStatus == 0 {
            ...
            rcordImg.image = UIImage.bundle(name: (String(notiAbsPath.prefix(6)) + String(showAttachStr)))
            //: rcordLB.text = "Received calls".localized
            rcordLB.text = (String(const_itsData.prefix(5)) + "ved ca" + String(app_numberSizeMessage)).localized
            //: } else {
        } else {
            //: rcordImg.image = UIImage.BundleImageNamed(name: "icon_dialedcalls")
            rcordImg.image = UIImage.bundle(name: (String(k_userTitle) + String(userMatchShareValue.prefix(4)) + "ledcalls"))
            //: rcordLB.text = "Dialed calls".localized
            rcordLB.text = (String(notiSizeStr.suffix(3)) + String(dataMakeMsg.suffix(4)) + "calls").localized
        }
...
extension OrdinalThen {
    ...
    private func clingOnReciprocation() {}
}
