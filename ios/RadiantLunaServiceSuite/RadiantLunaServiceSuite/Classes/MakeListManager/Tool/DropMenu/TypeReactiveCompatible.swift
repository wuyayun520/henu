...
fileprivate let userTotalerestData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "OptionCommentMenuCell" :*/
fileprivate let kEdgeKey:String = "video start sendDropM"
fileprivate let kViewArrayStr:[Character] = ["e","m","b","e","r","M","e","n","u","C","e","l","l"]

/*: "roomId" :*/
fileprivate let userNameUrl:String = "min if eventroomId"

/*: "type" :*/
fileprivate let kSectionMsg:String = "tyvideoe"

/*: "page" :*/
fileprivate let main_groupMsg:String = "pagicon"

/*: "icon_chats_pcr" :*/
fileprivate let main_toolName:[Character] = ["i","c","o","n","_","c","h","a","t","s","_","p","c","r"]

/*: "All Numbers" :*/
fileprivate let userModelShowFormat:[Character] = ["A","l","l"," ","N"]
fileprivate let main_talkValue:[Character] = ["u","m","b","e","r","s"]

/*: "Recent Gifts" :*/
fileprivate let show_tempTouchMessage:[Character] = ["R","e","c","e","n","t"," "]
fileprivate let appEqualUrl:String = "self model text rightGifts"
...
class TypeReactiveCompatible: UIView {
    //: var pageIndex = 0
    var pageIndex = 0
    //: var roomID = ""
    var roomID = ""
    ...
    fileprivate lazy var contentView: UIView = self.halfPhone()
    //: fileprivate lazy var mTable: UITableView = self.makeTableView()
    fileprivate lazy var mTable: UITableView = self.currentSelect()
    //: fileprivate var anchorRect = CGRect.zero
    fileprivate var anchorRect = CGRect.zero

    //: static let shared = DropDownMemberMenuView(frame: .zero)
    static let shared = TypeReactiveCompatible(frame: .zero)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userTotalerestData.reversed(), encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func halfPhone() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func currentSelect() -> UITableView {
    ...
    private func addFile() {
        ...
        mTable.register(OptionCommentMenuCell.self, forCellReuseIdentifier: (String(kEdgeKey.suffix(5)) + String(kViewArrayStr)))
    ...
    func bindHead() {
        ...
        var dict = [String: Any]()
        //: dict["roomId"] = roomID
        dict[(String(userNameUrl.suffix(6)))] = roomID
        //: dict["type"] = "3"
        dict[(kSectionMsg.replacingOccurrences(of: "video", with: "p"))] = "3"
        //: dict["page"] = String(pageIndex)
        dict[(main_groupMsg.replacingOccurrences(of: "icon", with: "e"))] = String(pageIndex)
        ...
        ObtrudeUponThen.share.smallness(params: dict, completion: { array in
            ...
            if self.pageIndex == 0 {
                ...
                let model = ChaseReactiveCompatible()
                //: model.headPic = "icon_chats_pcr"
                model.headPic = (String(main_toolName))
                //: model.nickname = "All Numbers".localized
                model.nickname = (String(userModelShowFormat) + String(main_talkValue)).localized
                //: model.uid = "all"
                model.uid = "all"
    ...
    private func weltanschauungPush() {
...
extension TypeReactiveCompatible: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(kEdgeKey.suffix(5)) + String(kViewArrayStr)), for: indexPath) as! OptionCommentMenuCell
        ...
        if recentmenuArray.count > 0 && indexPath.section == 0 {
            ...
            cellModel = recentmenuArray[indexPath.row] as! ChaseReactiveCompatible
            //: }else {
        } else {
            //: cellModel = menuArray[indexPath.row] as! TalkingChatRoomMemberModel
            cellModel = menuArray[indexPath.row] as! ChaseReactiveCompatible
        }
        //: cell.setDropMemberMenuCell(model: cellModel)
        cell.element(model: cellModel)
        //: return cell
        return cell
    }
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        menuDelegate?.bestowal(model: cellModel)
    }

    //: func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
    func tableView(_: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        //: let headeView = UILabel.init(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: 23))
        let headeView = UILabel(frame: CGRect(x: 0, y: 0, width: main_colorData, height: 23))
        //: headeView.backgroundColor = UIColor.appBgColor()
        headeView.backgroundColor = UIColor.pushAside()
        //: let titleLB = UILabel.init()
        let titleLB = UILabel()
        //: titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        //: titleLB.font = .pingfangFont(type: .Medium, fontSize: 12)
        titleLB.font = .replyName(type: .Medium, fontSize: 12)
        //: titleLB.textColor = .appValueDetailColor()
        titleLB.textColor = .valueColor()
        //: if recentmenuArray.count > 0 && section == 0 {
        if recentmenuArray.count > 0 && section == 0 {
            //: titleLB.text = "Recent Gifts".localized
            titleLB.text = (String(show_tempTouchMessage) + String(appEqualUrl.suffix(5))).localized
            //: }else {
        } else {
            //: titleLB.text = "All Numbers".localized
            titleLB.text = (String(userModelShowFormat) + String(main_talkValue)).localized
        }
