...
fileprivate let mainEditUrl:[UInt8] = [0x1c,0x1b,0x1c,0x1,0x5d,0x16,0x1a,0x11,0x10,0x7,0x4f,0x5c,0x55,0x1d,0x14,0x6,0x55,0x1b,0x1a,0x1,0x55,0x17,0x10,0x10,0x1b,0x55,0x1c,0x18,0x5,0x19,0x10,0x18,0x10,0x1b,0x1,0x10,0x11]

/*: "icon_photo_giftbg" :*/
fileprivate let noti_attributeKey:[Character] = ["i","c","o","n","_","p","h","o","t","o","_"]
fileprivate let showPathValue:String = "send"
fileprivate let notiSexUrl:String = "ifcornerg"

/*: "Send" :*/
fileprivate let appQuantityStatusEqualName:String = "normal for size objectSend"

/*: "nav_back_black_nor" :*/
fileprivate let k_toValue:String = "nav_baview view current color"
fileprivate let userLengthPathFormat:String = "voice time fee bottom rawlack_nor"

/*: "toUid" :*/
fileprivate let appRequestTitle:[Character] = ["t","o","U","i","d"]

/*: "giftId" :*/
fileprivate let appDuringMsg:String = "data ingiftId"

/*: "giftNum" :*/
fileprivate let dataEqualUrl:String = "giftNsection of"
fileprivate let app_maleServerKey:String = "uclear"

/*: "unlockMsgId" :*/
fileprivate let show_infoText:String = "tablelock"
fileprivate let constReplyName:String = "more layer borderMsgId"

/*: "Need a %@ (%@ Gold coins) to unlock" :*/
fileprivate let const_videoUrl:[UInt8] = [0x9c,0xb7,0xb7,0xb6,0xf2,0xb3,0xf2,0xf7,0x92,0xf2,0xfa,0xf7,0x92,0xf2,0x95,0xbd,0xbe,0xb6,0xf2,0xb1,0xbd,0xbb,0xbc,0xa1,0xfb,0xf2,0xa6,0xbd,0xf2,0xa7,0xbc,0xbe,0xbd,0xb1,0xb9]

/*: "icon_photo_lock" :*/
fileprivate let show_buttonFormat:[Character] = ["i","c","o","n","_","p","h","o","t","o","_","l","o","c","k"]

/*: "Click on the screen to view 15s" :*/
fileprivate let mainAddMessage:[UInt8] = [0x3f,0x10,0x15,0x1f,0x17,0x5c,0x13,0x12,0x5c,0x8,0x14,0x19,0x5c,0xf,0x1f,0xe,0x19,0x19,0x12,0x5c,0x8,0x13,0x5c,0xa,0x15,0x19,0xb,0x5c,0x4d,0x49,0xf]

/*: "icon_photo_burn" :*/
fileprivate let noti_styleTitle:[Character] = ["i","c","o","n","_","p","h","o","t"]
fileprivate let kSessionMessage:String = "app equal for leading stringo_burn"

/*: "Already burned" :*/
fileprivate let user_showMainTitle:[Character] = ["A","l","r","e","a","d"]
fileprivate let mainSmallStr:String = "name contenty burned"
...
// __CLOSE_PRINT__
//
//  GestureTitleViewController.swift
//  AbroadTalking
//
...
typealias IntimatePhotoBlock = (_ model: BlueprintModel) -> Void

//: class TalkingIntimatePhotoPreviewVC: TalkingBaseViewController {
class GestureTitleViewController: LightThen {
    ...
    //: private var imageTotalData = FountainheadCellData(direction: .MsgDirectionOutgoing)
    private var imageTotalData = FountainheadCellData(direction: .MsgDirectionOutgoing) // 图片消息数据
    //: private var photoModel = AbTalkingChatMsgIntimatePhotoModel()
    private var photoModel = BlueprintModel() // 当前私密照片模型
    ...
    ///   - imageCellData: 图片模型
    ///   - toUid: 对方Id
    //: init(imageCellData: FountainheadCellData, toUid: String) {
    init(imageCellData: FountainheadCellData, toUid: String) {
        ...
        //: targetId = toUid
        targetId = toUid
        //: isMySelf = (imageTotalData.msgModel.user.uid == Int(IconContainerAppManager.share.loginUserMode.userID))
        isMySelf = (imageTotalData.msgModel.user.uid == Int(IconContainerAppManager.share.loginUserMode.userID))
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainEditUrl.map{$0^117}, encoding: .utf8)!)
    }
    ...
    private lazy var giftBgView: UIImageView = {
        ...
        img.image = UIImage.bundle(name: (String(noti_attributeKey) + showPathValue.replacingOccurrences(of: "send", with: "g") + notiSexUrl.replacingOccurrences(of: "corner", with: "tb")))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftIcon: UIImageView = {
        ...
        img.pagePush(urlStr: photoModel.giftIcon)
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftSendBtn: UIButton = {
        ...
        btn.setTitle((String(appQuantityStatusEqualName.suffix(4))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(proposal), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var backBtn: UIButton = {
        ...
        let img = UIImage.bundle(name: (String(k_toValue.prefix(6)) + "ck_b" + String(userLengthPathFormat.suffix(8)))).withRenderingMode(.alwaysTemplate)
...
extension GestureTitleViewController {
    /// 调用送礼接口
    /// - Parameters:
    ///   - giftId: 礼物Id
    ///   - giftNum: 礼物数量
    //: func req_sendGiftMsg(giftId: String, giftNum: String) {
    func giftNum(giftId: String, giftNum: String) {
        ...
        params[(String(appRequestTitle))] = self.targetId
        //: params["giftId"] = giftId
        params[(String(appDuringMsg.suffix(6)))] = giftId
        //: params["giftNum"] = giftNum
        params[(String(dataEqualUrl.prefix(5)) + app_maleServerKey.replacingOccurrences(of: "clear", with: "m"))] = giftNum
        //: params["unlockMsgId"] = self.imageTotalData.msgModel.msgInfo.msgId
        params[(show_infoText.replacingOccurrences(of: "table", with: "un") + String(constReplyName.suffix(5)))] = self.imageTotalData.msgModel.msgInfo.msgId
...
extension GestureTitleViewController {
    ...
    @objc private func proposal() {
        ...
        self.giftNum(giftId: photoModel.giftId, giftNum: "1")
    }
    ...
    @objc private func burn() {
        ...
        self.inserter(urlStr: photoModel.originUrl, isMosaic: false) { [weak self] isSucceed in
            //: ProgressHUD.dismiss()
            ...
            self.countdownTimer = Timer.scheduledTimer(withTimeInterval: 0.1, block: { [weak self] _ in
                ...
                self.progressView.moralsPush(totalTime: 150, currTime: time)
                //: time -= 1
                time -= 1
                //: }, repeats: true)
            }, repeats: true)
...
extension GestureTitleViewController {
    ...
    private func componentInSocial() {
        //: self.maskView.isHidden = true
        self.maskView.isHidden = true
        //: self.desLab.isHidden = true
        self.desLab.isHidden = true
        ...
        if photoModel.lockStatus == 1 { // 锁定状态，需要送礼解锁
            ...
            self.desLab.text = String(bytes: const_videoUrl.map{$0^210}, encoding: .utf8)!.cellArguments(photoModel.giftName, photoModel.giftPrice)
            ...
            if photoModel.burnStatus == 0 { // 未销毁
                ...
                self.intimateStatusImgV.image = UIImage.bundle(name: (String(show_buttonFormat)))
                //: self.desLab.text = "Click on the screen to view 15s".localized
                self.desLab.text = String(bytes: mainAddMessage.map{$0^124}, encoding: .utf8)!.localized
                // 添加手势
                ...
                self.intimateStatusImgV.image = UIImage.bundle(name: (String(noti_styleTitle) + String(kSessionMessage.suffix(6))))
                //: self.desLab.text = "Already burned".localized
                self.desLab.text = (String(user_showMainTitle) + String(mainSmallStr.suffix(8))).localized
            }
        ...
        self.inserter(urlStr: imgUrlStr, isMosaic: imgIsMosaic)
    }
    ...
    private func inserter(urlStr: String, isMosaic: Bool, completionHandler: ((_ isSucceed: Bool) -> Void)? = nil) {
