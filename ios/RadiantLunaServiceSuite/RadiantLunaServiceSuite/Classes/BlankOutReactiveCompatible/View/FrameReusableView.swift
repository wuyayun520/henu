...
fileprivate let data_frameMessage:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
let appBeautyData = actualWidth(w: 96)

//: class TalkingDailyHeaderView: UICollectionReusableView {
class FrameReusableView: UICollectionReusableView {
    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: data_frameMessage.reversed(), encoding: .utf8)!)
    }
...
extension FrameReusableView {
    ...
    private func subviewsSend() {
        ...
        self.meanSunRadii(width: main_colorData, height: appBeautyData, corners: [.topLeft, .topRight], cornerRadii: .init(width: 16, height: 16))
        ...
        for (index, title) in weekArr.enumerated() {
            ...
            let width = main_colorData / CGFloat(weekArr.count)
            //: if index == 0 {
            if index == 0 { // 首个
