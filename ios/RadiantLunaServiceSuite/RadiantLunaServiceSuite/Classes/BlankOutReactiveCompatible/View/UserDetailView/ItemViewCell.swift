...
fileprivate let showSizeInputData:[UInt8] = [0xc,0xb,0xc,0x11,0x4d,0x6,0xa,0x1,0x0,0x17,0x5f,0x4c,0x45,0xd,0x4,0x16,0x45,0xb,0xa,0x11,0x45,0x7,0x0,0x0,0xb,0x45,0xc,0x8,0x15,0x9,0x0,0x8,0x0,0xb,0x11,0x0,0x1]

				private func biologyLaboratory(sun num: UInt8) -> UInt8 {
					return num ^ 101
				}

/*: "More" :*/
fileprivate let constInfoText:String = "Moreview target with not"

/*: "icon_details_arrow" :*/
fileprivate let notiEditUrl:[Character] = ["i","c","o","n","_","d","e","t","a"]
fileprivate let show_layerData:[Character] = ["i","l"]
fileprivate let k_messageName:String = "s_arrowprofile cell view"

/*: "Post" :*/
fileprivate let user_buttonMakeBlockUrl:String = "style min topPost"

/*: "btn_dynamic_stop_nor" :*/
fileprivate let user_addReloadStr:String = "btn_result succeed who"
fileprivate let data_equalId:String = "c_stop_npoint else point"
fileprivate let appViewMsg:String = "oscreen"
...
class ItemViewCell: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showSizeInputData.map{biologyLaboratory(sun: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleLab: UILabel = {
        ...
        lb.font = .replyName(type: .Medium, fontSize: 17)
    ...
    lazy var maskMoreView: UIView = {
        ...
        view.backgroundColor = .roundVoice(0, 0, 0, 0.4)
    ...
    lazy var moreButton: BlockShouldShirtButton = .init().then {
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 17)
        $0.titleLabel?.font = .replyName(type: .Regular, fontSize: 17)
        //: $0.setTitle("More".localized, for: .normal)
        $0.setTitle((String(constInfoText.prefix(4))).localized, for: .normal)
        //: $0.setImage(UIImage(named: "icon_details_arrow"), for: .normal)
        $0.setImage(UIImage(named: (String(notiEditUrl) + String(show_layerData) + String(k_messageName.prefix(7)))), for: .normal)
...
extension ItemViewCell {
    ...
    func dataUp(userModel: CustomHandyJSON) {
        ...
        strA = [(String(user_buttonMakeBlockUrl.suffix(4))).localized, str]
        //: colorA = [UIColor.appTitleColor(), UIColor.appValueColor()]
        colorA = [UIColor.sendTitle(), UIColor.colorData()]
        //: fontA = [UIFont.pingfangMediumFont(fontSize: 18), UIFont.pingfangMediumFont(fontSize: 16)]
        fontA = [UIFont.font(fontSize: 18), UIFont.font(fontSize: 16)]
        ...
        for index in 0 ..< itemCount {
            ...
            if index == 0 {
                ...
                let model: UserTipMeasurable = (userModel.momentPic?.list)![index]
                ...
                firstImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.fromSquare())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: firstImgView)
                    intervalimate(btn: firstImgView)
                }
                ...
                secondImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.fromSquare())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: secondImgView)
                    intervalimate(btn: secondImgView)
                }
                //: } else {
            } else {
                //: let model: TalkingUserInfoMomentItemModel = (userModel.momentPic?.list)![index]
                ...
                thirdImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.fromSquare())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: thirdImgView)
                    intervalimate(btn: thirdImgView)
                }
            }
        }
    }
    ...
    @objc func by() {
        ...
        let vc = EffectViewController(uid: userId)
    ...
    func intervalimate(btn: UIImageView) {
        //: btn.subviews.map {
        btn.subviews.map {
            //: (view) -> () in
            view in
        ...
        let imag = UIImageView(image: UIImage(named: (String(user_addReloadStr.prefix(4)) + "dynami" + String(data_equalId.prefix(8)) + appViewMsg.replacingOccurrences(of: "screen", with: "r"))))
...
extension ItemViewCell {
    ...
    private func dataConstraint() {
        ...
        let height = (main_colorData - actualWidth(w: 15 * 4)) / 3.0
