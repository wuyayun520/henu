...
fileprivate let k_mediaMessage:[UInt8] = [0x2a,0x2f,0x2a,0x35,0xe9,0x24,0x30,0x25,0x26,0x33,0xfb,0xea,0xe1,0x29,0x22,0x34,0xe1,0x2f,0x30,0x35,0xe1,0x23,0x26,0x26,0x2f,0xe1,0x2a,0x2e,0x31,0x2d,0x26,0x2e,0x26,0x2f,0x35,0x26,0x25]

				fileprivate func partyBorder(to num: UInt8) -> UInt8 {
...
/*: "icon_me_voice_white" :*/
fileprivate let userEffectDataText:[Character] = ["i","c","o","n","_","m","e","_","v"]
fileprivate let showViewPathText:String = "row"
fileprivate let notiBackgroundTitle:String = "instance rating viewice_"

/*: "btn_auto_write" :*/
fileprivate let showRestoreText:[Character] = ["b","t","n","_","a"]
fileprivate let main_touchMessage:String = "umake"
fileprivate let main_modelText:String = "o_writesize video"

/*: "Pending review" :*/
fileprivate let show_serverPath:[Character] = ["P","e","n","d","i","n","g"]
fileprivate let userRequestTitle:[Character] = [" "]
fileprivate let show_toKey:String = "rtextvitextw"

/*: "get json error" :*/
fileprivate let const_quoteData:String = "equal recordingget js"
fileprivate let showLiveMessage:String = "oadd"
...
class StreamViewThen: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_mediaMessage.map{partyBorder(to: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var statusLb: UILabel = {
        ...
        lb.textColor = UIColor.tipsRender()
        //: return lb
        return lb
        //: }()
    }()
    ...
    lazy var audioBtn: UIButton = {
        ...
        btn.setBackgroundImage(UIImage.smallSize(colors: UIColor.equalAction(), size: CGSize(width: 155, height: 35)), for: .normal)
        //: btn.layer.cornerRadius = 17.5
        btn.layer.cornerRadius = 17.5
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(recordBtnAction), for: .touchUpInside)
        btn.addTarget(self, action: #selector(makeSearched), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var playIconView: UIImageView = {
        ...
        let imag = UIImageView(image: UIImage.bundle(name: (String(userEffectDataText) + showViewPathText.replacingOccurrences(of: "row", with: "o") + String(notiBackgroundTitle.suffix(4)) + "white")))
        //: return imag
        return imag
        //: }()
    }()
    ...
    lazy var editBtn: UIButton = {
        ...
        btn.setImage(UIImage.bundle(name: (String(showRestoreText) + main_touchMessage.replacingOccurrences(of: "make", with: "t") + String(main_modelText.prefix(7)))), for: .normal)
        //: btn.addTarget(self, action: #selector(editBtnAction), for: .touchUpInside)
        btn.addTarget(self, action: #selector(prohibition), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension StreamViewThen {
    //: @objc func recordBtnAction() {
    @objc func makeSearched() {
    ...
    func audio(model: SizeTransformable, row: Int, isLast: Bool) {
        //: index = row
        index = row
        //: islast = isLast
        islast = isLast
        ...
        if model.status == 0 {
            ...
            statusLb.text = (String(show_serverPath) + String(userRequestTitle) + show_toKey.replacingOccurrences(of: "text", with: "e")).localized
    ...
    func achromaticColor() {
        //: do {
        do {
            //: let url = SVGAEffectTool.default.getZipEffectPath(type: .Play_Audio_Wave)
            let url = ThemeReactiveCompatible.default.windowMake(type: .Play_Audio_Wave)
            ...
            printLog(message: (String(const_quoteData.suffix(6)) + "on err" + showLiveMessage.replacingOccurrences(of: "add", with: "r")))
        }
    }
}
