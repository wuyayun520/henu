...
fileprivate let dataTitleId:[UInt8] = [0x96,0xaa,0xab,0xb5,0x62,0xa8,0xa7,0xa3,0xb6,0xb7,0xb4,0xa7,0x62,0xab,0xb5,0x62,0xb0,0xb1,0xb6,0x62,0xa3,0xb8,0xa3,0xab,0xae,0xa3,0xa4,0xae,0xa7,0x62,0xa6,0xb7,0xb4,0xab,0xb0,0xa9,0x62,0xa3,0x62,0xa5,0xa3,0xae,0xae]

				fileprivate func backBroadcast(video num: UInt8) -> UInt8 {
...
protocol StreamAppToolDelegate: NSObject {
    /// 录音时间
...
class CommunicationAudioTool: NSObject {
    //: open weak var delegate: TalkingAudioToolDelegate?
    open weak var delegate: StreamAppToolDelegate?
    ...
    static let shared = CommunicationAudioTool()
    ...
    override private init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(modelNotification(notification:)), name: AVAudioSession.interruptionNotification, object: nil)
    }

    //: func getVoiceRecordLocalPathWithFileType(path: NSString) -> String {
    func bag(path: NSString) -> String {
...
extension CommunicationAudioTool: AVAudioRecorderDelegate, AVAudioPlayerDelegate {
    /// 录制、播放过程中中断
    ...
    @objc func modelNotification(notification: NSNotification) {
        ...
        if changeReason?.intValue ?? 0 == AVAudioSession.InterruptionType.began.rawValue {
            ...
            self.explain(showMsg: String(bytes: dataTitleId.map{backBroadcast(video: $0)}, encoding: .utf8)!.localized)
            //: if self.isRecording {
            if self.isRecording {
                //: self.delegate?.func__recordFinishWithFile(filePath: self.filePath)
                self.delegate?.pendingInstance(filePath: self.filePath)
...
//                destroyRecordTimer()
                ...
                self.delegate?.textDetail(status: .PlayerInterruption)
            }
        }
    }
    ...
    func audioPlayerDidFinishPlaying(_: AVAudioPlayer, successfully _: Bool) {
        ...
        self.delegate?.textDetail(status: .PlayerFinish)
    }
}

//: extension TalkingAudioTool {
extension CommunicationAudioTool {
    //: func BeginRecord() {
    func talk() {
        //: deleteFile()
        fromFile()
    ...
    func page() {
        ...
        recordTimer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(authorSName(time:)), userInfo: nil, repeats: true)
    ...
    @objc func authorSName(time _: Timer) {
        //: recorder?.updateMeters()
        recorder?.updateMeters()
        ...
        self.delegate?.cancel(currentTime: interval)
    }
    ...
    func visualizationTask() {
        ...
        self.delegate?.abdicate(currentTime: currentTime)
    }
    ...
    func viewDetailRed() {
        ...
        playTimer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(content(time:)), userInfo: nil, repeats: true)
        //: if let curTime: Timer = recordTimer {
        if let curTime: Timer = recordTimer {
            //: RunLoop.main.add(curTime, forMode: .common)
            RunLoop.main.add(curTime, forMode: .common)
    ...
    @objc func content(time _: Timer) {
        //: if self.player != nil {
        if self.player != nil {
