...
struct TextToModelType: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
struct AtModelType: HandyJSON {
    ...
    var status: ViewMirrorPath?
}

///
//: enum MomentVideoStatus: String {
enum ViewMirrorPath: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension AtMeasurable {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func contrive(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(CustomThen.number(text: content!, size: CGSize(width: main_colorData - 54, height: CGFloat(MAXFLOAT)), font: UIFont.replyName(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == MomentConvertible.Photo.rawValue || momentType == MomentConvertible.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
