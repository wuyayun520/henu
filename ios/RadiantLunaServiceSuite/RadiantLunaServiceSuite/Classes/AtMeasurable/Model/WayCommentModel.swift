...
/*: "reply" :*/
fileprivate let appModelName:String = "repcelly"
...
struct WayCommentModel: HandyJSON {
    //: var cid: String?
    ...
    var comment = [NameTipTransformable]()
...
extension WayCommentModel {
    //: mutating func caculateCommentItemHeight() {
    mutating func instanceHeight() {
        //: var tempContent = ""
        var tempContent = ""
        ...
        if replyUser?.count ?? 0 > 0 {
            ...
            if GlassLanguageManager.shared.direction == .leftToRight {
                ...
                tempContent = (appModelName.replacingOccurrences(of: "cell", with: "l")).localized + "\(replyUser!): \(content!)"
                //: } else {
            } else {
                //: tempContent = "\(content!) :\(replyUser!)" + "reply".localized
                tempContent = "\(content!) :\(replyUser!)" + (appModelName.replacingOccurrences(of: "cell", with: "l")).localized
            }
        ...
        let sizeheight = Int(CustomThen.number(text: tempContent, size: CGSize(width: main_colorData - 70, height: CGFloat(MAXFLOAT)), font: UIFont.replyName(type: .Regular, fontSize: 16), lineNumber: 0).height)
